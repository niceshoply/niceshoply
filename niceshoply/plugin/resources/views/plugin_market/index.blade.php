...
@section('title', __('console/common.market_plugin'))
      ...
      <div class="mb-4">
        ...
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
          ...
          <div class="flex-shrink-0">
            <form method="GET" action="{{ console_route('plugin-market.index') }}" class="d-flex" id="searchForm">
              <div class="input-group input-group-sm search-input-group">
                ...
                <input type="text" 
                       ...
                       placeholder="{{ __('console/common.search') }}..." 
                       id="searchInput"
                       autocomplete="off">
                @if(request()->get('search'))
                  <button type="button" class="btn btn-outline-secondary border-start-0" id="clearSearch" title="{{ __('console/common.clear_selection') }}">
                    <i class="bi bi-x"></i>
                  </button>
                @endif
              </div>
        ...
        <div class="mb-3">
          @if($categories['data'] ?? [])
            ...
            <div class="d-flex align-items-center gap-2 flex-wrap">
              <span class="text-muted small fw-medium flex-shrink-0">{{ __('console/common.category') }}:</span>
              <div class="d-flex gap-2 flex-wrap align-items-center">
                ...
                @if($hasMore)
                  <button type="button" class="btn btn-sm btn-outline-secondary rounded px-3" id="toggleMoreCategories">
                    <span class="show-more-text">{{ __('console/common.view_more') }}</span>
                    <span class="show-less-text d-none">{{ __('console/common.collapse') }}</span>
                  </button>
                @endif
              </div>
            </div>
        ...
        @if(isset($products['meta']))
          <div class="mb-3">
            <p class="text-muted small mb-0">
              {{ __('console/common.total_records', ['total' => $products['meta']['total'] ?? 0, 'current' => $products['meta']['current_page'] ?? 1, 'last' => $products['meta']['last_page'] ?? 1]) }}
            </p>
          </div>
        @endif
      </div>
          ...
          @if(empty($products['data']))
            <div class="text-center py-5">
              ...
              <p class="text-muted mt-3 mb-0">{{ __('console/common.no_data') }}</p>
            </div>
          @else
