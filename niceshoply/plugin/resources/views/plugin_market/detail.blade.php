...
@section('title', __('console/menu.plugin_market'))
...
@section('content')
  ...
  @if(session('error') || isset($error))
    <div class="alert alert-danger alert-dismissible fade show" role="alert">
      <strong>{{ __('console/common.error') }}:</strong> {{ session('error') ?? $error ?? '' }}
        ...
        <div class="col-12">
                  ...
                  <div class="col-md-8">
                    ...
                    <div class="d-flex flex-wrap gap-4 mb-4">
                      @if($product['viewed'] ?? 0)
                        <span class="text-muted">
                          <i class="bi bi-eye me-1"></i>{{ __('console/plugin.views') }}: <strong>{{ $product['viewed'] }}</strong>
                        </span>
                      @endif
                      @if($product['downloaded'] ?? 0)
                        <span class="text-muted">
                          <i class="bi bi-download me-1"></i>{{ __('console/plugin.downloads') }}: <strong>{{ $product['downloaded'] }}</strong>
                        </span>
                      @endif
                      @if($product['updated_at'] ?? '')
                        <span class="text-muted">
                          <i class="bi bi-clock me-1"></i>{{ __('console/plugin.last_updated') }}: <strong>{{ $product['updated_at'] }}</strong>
                        </span>
                      @endif
                    </div>
                    ...
                    <div class="card bg-light border-0 p-4 mb-4">
                      <div class="row g-4">
                        <div class="col-md-6">
                          <div class="mb-3">
                            <span class="text-muted d-block mb-1">{{ __('console/plugin.version') }}</span>
                            <span class="fw-bold fs-5">{{ $product['version'] ?? 'N/A' }}</span>
                          </div>
                        </div>
                        <div class="col-md-6">
                          <div class="mb-3">
                            <span class="text-muted d-block mb-1">{{ __('console/plugin.author') }}</span>
                        ...
                        <div class="col-12">
                          <div class="border-top pt-3">
                            <span class="text-muted d-block mb-2">{{ __('console/plugin.price') }}</span>
                    ...
                    @if($product['downloadable'] ?? false)
                      <button class="btn btn-primary btn-lg px-5" type="button" id="btnDownload" data-product-type="plugin">
                        <i class="bi bi-download me-2"></i>{{ __('console/plugin.download_now') }}
                      </button>
                    @else
                      ...
                      <div class="mb-3">
                        <button class="btn btn-danger btn-lg px-5" id="quickBuy" data-sku-id="{{$product['sku_id']}}"
                          ...
                          <i class="bi bi-cart-plus me-2"></i>{{ __('console/plugin.buy_now') }}
                        </button>
                      </div>
                      <div class="alert alert-warning border-0 mb-0" style="background-color: #fff3cd;">
                        <small class="d-block mb-1"><strong>{{ __('console/plugin.purchase_notice') }}:</strong></small>
                        <small class="d-block mb-1">1. {{ __('console/plugin.notice_multi_domain') }}</small>
                        <small class="d-block mb-1">2. {{ __('console/plugin.notice_no_refund') }}</small>
                        <small class="d-block">3. {{ __('console/plugin.notice_support_expiry') }}</small>
                      </div>
                    @endif
                  </div>
                </div>
                ...
                <ul class="nav nav-tabs border-bottom mb-4" role="tablist">
                  <li class="nav-item" role="presentation">
                    <button class="nav-link active" id="introduction-tab" data-bs-toggle="tab" data-bs-target="#introduction" type="button" role="tab" aria-controls="introduction" aria-selected="true">
                      <i class="bi bi-info-circle me-2"></i>{{ __('console/plugin.plugin_introduction') }}
                    </button>
                  </li>
                  @if($product['content'] ?? '')
                    <li class="nav-item" role="presentation">
                      <button class="nav-link" id="details-tab" data-bs-toggle="tab" data-bs-target="#details" type="button" role="tab" aria-controls="details" aria-selected="false">
                        <i class="bi bi-file-text me-2"></i>{{ __('console/plugin.details') }}
                      </button>
                    </li>
                  @endif
                </ul>
          ...
          @else
            <div class="alert alert-warning" role="alert">
              {{ __('console/common.product_not_found') }}
            </div>
          @endif
        </div>
      </div>
    ...
    <script>
        $(function () {
            ...
            $('#btnDownload').click(function (){
                axios({
                    method: "get",
                    url: '{{ console_route('marketplaces.download', ['id' => $product['product_id'] ?? 0, 'type' => 'plugin']) }}',
                    ...
                    layer.msg(err.response?.data?.message || err.message || '{{ __('console/common.error') }}', { icon: 2 })
                })
            })
            @endif
        })
