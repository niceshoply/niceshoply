<div class="my-4" id="billingMethodsContent">
    <h5 class="mb-3">{{ __('console/plugin.payment_method') }}</h5>
    ...
    <script>
        $(function () {
            ...
            var paymentMethodTranslations = {
                'alipay': '{{ __('console/plugin.payment_alipay') }}',
                'alipay_payment': '{{ __('console/plugin.payment_alipay') }}',
                'wechatpay': '{{ __('console/plugin.payment_wechatpay') }}',
                'wechat_pay': '{{ __('console/plugin.payment_wechat_pay') }}',
                'we_chat_pay': '{{ __('console/plugin.payment_wechat_pay') }}',
                'wechat': '{{ __('console/plugin.payment_wechatpay') }}',
            };
            ...
            axios.get("{{ route('api.checkout.billing_methods') }}").then(function (res) {
            ...
            $('#quickBuy').on('click',function () {
                if (billingMethod === '') {
                    layer.msg('{{ __('console/plugin.payment_method_confirm') }}', {icon: 2})
                    return
                }
                axios.post('/{{ console_name() }}/marketplaces/quick_checkout', {
                    ...
                    window.open(res.data.payment_url || ('{{config('niceshoply.api_url')}}' + '/orders/' + res.data.number + '/pay'), '_blank')
                })
            })
        })
    </script>
