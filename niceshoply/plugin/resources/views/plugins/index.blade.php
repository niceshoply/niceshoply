...
@section('title', $type ? __('console/plugin.'.$type) : __('console/plugin.all'))
      ...
      <ul class="nav nav-tabs mb-4" role="tablist">
        <li class="nav-item" role="presentation">
          <a class="nav-link {{ !$type ? 'active' : '' }}" 
             href="{{ console_route('plugins.index') }}">
            {{ __('console/plugin.all') }}
        ...
        @foreach($types as $pluginType)
          <li class="nav-item" role="presentation">
            <a class="nav-link {{ $type === $pluginType ? 'active' : '' }}" 
               href="{{ console_route('plugins.index', ['type' => $pluginType]) }}">
              {{ __('console/plugin.'.$pluginType) }}
              ...
              <div class="plugin-info">
                ...
                <div class="d-flex justify-content-between align-items-center">
                  ...
                  <div class="btns">
                    @if ($plugin['installed'])
                      @if($plugin['menu_url'])
                        <a href="{{ $plugin['menu_url'] }}" class="btn btn-success btn-sm" title="{{ __('console/common.use') }}">
                          <i class="bi bi-box-arrow-up-right"></i> {{ __('console/common.use') }}
                        </a>
                      @endif
                      @if($plugin['edit_url'])
                        <a href="{{ $plugin['edit_url'] }}" class="btn btn-primary btn-sm">{{ __('console/common.edit') }}</a>
                      @endif
                      <div class="btn btn-danger btn-sm uninstall-plugin">{{ __('console/common.uninstall') }}</div>
                    @else
                      <div class="btn btn-primary btn-sm install-plugin">{{ __('console/common.install') }}</div>
                    @endif
                  </div>
                </div>
              </div>
...
@push('footer')
  <script>
    ...
    $('.plugin-enabled-switch input').change(function () {
      ...
      axios.post('/{{ console_name() }}/plugins/enabled', {code: code, enabled: enabled}).then(function (res) {
        if (res.success) {
    ...
    function pluginsUpdate(code, type) {
      const url = type === 'install' ? '/{{ console_name() }}/plugins' : '/{{ console_name() }}/plugins/' + code;
