  ...
  <div class="card-body">
      ...
      <div class="col ps-0">
        ...
        <div class="d-flex flex-wrap gap-3 align-items-center">
          <div class="text-secondary small">
            <i class="bi bi-person"></i> {{ __('console/plugin.author') }}: {{ $plugin->getAuthorName() }}
            @if($plugin->getAuthorEmail())
              <span class="ms-2"><i class="bi bi-envelope"></i> {{ __('console/plugin.email') }}: <a href="mailto:{{ $plugin->getAuthorEmail() }}" class="text-decoration-none">{{ $plugin->getAuthorEmail() }}</a></span>
            @endif
          </div>
          @if($plugin->getType())
            <div class="text-secondary small"><i class="bi bi-tag"></i> {{ __('console/plugin.type') }}：{{ $plugin->getTypeFormat() }}</div>
          @endif
          @if($plugin->checkInstalled() && $plugin->getEnabled() && $plugin->getMenuUrl())
            <div class="ms-auto">
              <a href="{{ $plugin->getMenuUrl() }}" class="btn btn-success btn-sm" title="{{ __('console/common.use') }}">
    ...
    <ul class="nav nav-tabs mt-4" role="tablist">
      <li class="nav-item">
        <button class="nav-link active" data-bs-toggle="tab" data-bs-target="#config-tab" role="tab">
          <i class="bi bi-gear"></i> {{ trans('console/plugin.config_settings') }}
        </button>
      </li>
      @if(!empty($readmeHtml) && trim($readmeHtml) !== '')
      <li class="nav-item">
        <button class="nav-link" data-bs-toggle="tab" data-bs-target="#readme-tab" role="tab">
          <i class="bi bi-book"></i> {{ trans('console/plugin.usage_documentation') }}
        </button>
      </li>
      @endif
    </ul>
    ...
    <div class="tab-content mt-3">
      <div class="tab-pane fade show active" id="config-tab" role="tabpanel">
        ...
        @else
          <form class="needs-validation" id="app-form" novalidate action="{{ console_route('plugins.update', [$plugin->getCode()]) }}" method="POST">
            @csrf
            {{ method_field('put') }}
            <div class="row">
