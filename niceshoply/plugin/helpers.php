...
use NiceShoply\Plugin\Core\Blade\Hook;
...
if (! function_exists('add_hook_action')) {
    /**
     * Alias for listen_hook_action, used in system internal code.
     *
     * @param  $hookName
     * @param  $callback
     * @param  int  $priority
     ...
     * @return void
     */
    function add_hook_action($hookName, $callback, int $priority = 20, int $arguments = 1): void
    {
        app('eventy')->addAction($hookName, $callback, $priority, $arguments);
    }
}
...
if (! function_exists('plugin_caller_priority')) {
    ...
    function plugin_caller_priority(): int
    {
        try {
            ...
            $pluginCode = str_replace(['Plugin\\', '\\Boot'], '', $class);
