...
namespace NiceShoply\DevTools\Console\Commands;
...
class InitPluginGit extends Command
{
     ...
     * @var string
     */
    protected $description = 'Initialize Git repository for a plugin and push to niceshoply.com';
    ...
    public function handle(): int
    {
        ...
        if (empty($giteaUrl)) {
            $giteaUrl = env('GITEA_URL', 'https://niceshoply.com');
        ...
        try {
            ...
            // 2. Set remote URL
            $remoteUrl = "git@niceshoply.com:splugins/{$pluginName}.git";
    ...
    private function createGiteaRepository(string $giteaUrl, string $token, string $org, string $repoName, bool $isPrivate = true): void
    {
        ...
        $requestData = [
            'name'        => $repoName,
            'description' => "NiceShoply Plugin: {$repoName}",
            'private'     => $isPrivate,
            'auto_init'   => false,
        ];
