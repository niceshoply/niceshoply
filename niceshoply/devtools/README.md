# NiceShoply 开发者工具包 (DevTools)

NiceShoply 开发者工具包是一个强大的命令行工具集，用于快速开发、验证和发布 NiceShoply 插件和主题。它提供了完整的脚手架生成、规范验证和市场发布功能。
...
- **规范检查**：发布前自动验证插件/主题规范
- **一键上传**：直接发布到 NiceShoply 官方市场

## 🚀 安装

DevTools 已集成到 NiceShoply 系统中，无需额外安装。如果作为独立 Composer 包使用：

```bash
composer require NiceShoply/devtools
```
...
**选项：**
- `--gitea-url`: Gitea 服务器地址（默认：`https://niceshoply.com`）
...
初始化插件的 Git 仓库并推送到 niceshoply.com。

**参数：**
- `plugin` (必需): 插件文件夹名称

**选项：**
- `--gitea-url`: Gitea 服务器地址（默认：从 `.env` 或 `storage` 读取，或 `https://niceshoply.com`）
...
**功能：**
1. 初始化 Git 仓库（如果不存在）
2. 设置远程仓库地址：`git@niceshoply.com:splugins/{plugin}.git`
...
**A:** 模板文件位于 `niceshoply/devtools/src/Templates/`，你可以修改这些模板文件来自定义生成的内容。
...
- [NiceShoply 开发规范](../docs/dev_standard.md)
...
**NiceShoply DevTools** - 让插件和主题开发更简单！🚀
