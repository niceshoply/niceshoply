...
class Creator
{
    ...
    {
        $appUrl     = detect_app_base_url();
        $forceHttps = str_starts_with(strtolower($appUrl), 'https://');
        $dbType     = strtolower($data['db_type']);

        $envFileData = 'APP_NAME='.($data['app_name'] ?? 'NiceShoply')."\n".
            'APP_ENV='.($data['environment'] ?? 'local')."\n".
            'APP_KEY='.'base64:'.base64_encode(Str::random(32))."\n".
            'APP_DEBUG=false'."\n".
            'APP_TIMEZONE=UTC'."\n".
            'APP_URL='.$appUrl."\n".
            ($forceHttps ? 'APP_FORCE_HTTPS=true'."\n" : '').
            "\n".
            'APP_LOCALE=en'."\n\n".
            // 后台 GlobalConsoleData 依赖 JWT；安装时必须写入，否则登录后 /console 500
            'JWT_SECRET='.Str::random(64)."\n\n";
        if ($dbType == 'mysql') {
            $envFileData .= 'DB_CONNECTION=mysql'."\n".
                'DB_PREFIX='.($data['db_prefix'] ?: 'nice_')."\n".
