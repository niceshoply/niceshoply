...
<head>
  ...
  <link rel="stylesheet" href="{{ build_asset('install/css/app.css') }}">
    ...
    <div class="install-wrap">
      <div class="install-1 install-item active">
        ...
        <div class="install-content" id="content">
          @php
            $licenseLocale = str_replace('-', '_', $locale);
          @endphp
          @if (view()->exists("install::license.{$licenseLocale}"))
            @include("install::license.{$licenseLocale}")
          @else
      ...
      <div class="install-3 install-item d-none">
        ...
        <div class="install-content">
          <form class="needs-validation" novalidate id="install-form">
            @csrf
            <input type="hidden" name="db_type" id="db_type_hidden" value="">
            ...
            <div class="row gx-2">
              ...
              <div class="col-6 mysql-item">
                <div class="mb-3">
                  ...
                  <input type="text" class="form-control" id="database" name="db_name" required value="niceshoply"
              ...
              <div class="col-6">
                <div class="mb-3">
                  ...
                  <input type="text" class="form-control" id="db_prefix" name="db_prefix" value="nice_" required
                ...
                <div class="col-6">
                  <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="admin_email" name="admin_email" required
                           placeholder="{{ __('install/common.admin_account') }}" value="admin@niceshoply.com">
      ...
      <div class="install-4 install-item install-success d-none">
        ...
        <div class="d-flex justify-content-center mt-4">
          ...
          <a href="{{ url(console_name()) }}" class="btn btn-primary">{{ __('install/common.visit_backend') }}</a>
        </div>
      </div>
    </div>
  </div>
...
<script>
  ...
  // Global variables
  const Installer = {
    mysqlInputTimer: null,

    // 当前步骤的「下一步」按钮（避免误操作其他步骤的按钮）
    getActiveNextBtn() {
      return $('.install-item.active .next-btn');
    },
    ...
    handleNextStep(e) {
      ...
      if (next.hasClass('install-3')) {
        $('#db_type_hidden').val($('#db-driver').val());
        $('.sql-type').val($('#db-driver').val()).prop('disabled', true).trigger('change');
        // 进入参数配置时，若 MySQL 字段已填则自动检测连接
        if ($('#db-driver').val() === 'mysql' && this.isMysqlFormReady()) {
          this.checkConnect();
        }
      }
      ...
      if (current.hasClass('install-3')) {
        ...
        $('#db_type_hidden').val($('.sql-type').val() || $('#db-driver').val());
        ...
        if (!form[0].checkValidity()) {
          ...
          if (firstInvalid.length) {
            ...
            layer.msg('{{ __('install/common.param_config') }}: {{ __('install/common.check_system') }}');
          }
          return;
        }

        const data = form.serialize();
        const nextStep = current.next('.install-item');
        this.checkComplete(data, () => this.activeStep(current, nextStep));
        return;
      }
    ...
    handlePrevStep(e) {
      ...
      this.getActiveNextBtn().prop('disabled', false);
    ...
    handleDbTypeChange(e) {
      ...
      const nextBtn = this.getActiveNextBtn();
      ...
      if (type === 'sqlite') {
        ...
        nextBtn.prop('disabled', !this.isMysqlFormReady());
        if (this.isMysqlFormReady()) {
          this.checkConnect();
        }
      }
    },

    // MySQL 必填项是否已填写（密码可为空）
    isMysqlFormReady() {
      return $('.mysql-item input').toArray().every(input => {
        const $input = $(input);
    ...
    handleMysqlInput(e) {
      if (!this.isMysqlFormReady()) {
        this.getActiveNextBtn().prop('disabled', true);
        return;
      }
      clearTimeout(this.mysqlInputTimer);
      this.mysqlInputTimer = setTimeout(() => this.checkConnect(), 500);
    },
    ...
    checkConnect() {
      $.ajax({
        ...
        data: {
          ...
          db_type: $('#db_type_hidden').val() || $('select[name="db_type"]').val(),
        ...
        success: (res) => {
          if (res.db_success) {
            ...
            setTimeout(() => {
              const $content = $('.install-3 .install-content');
              $content.animate({ scrollTop: $content[0].scrollHeight }, 400);
            }, 200);
          } else {
            this.getActiveNextBtn().prop('disabled', true);
            for (const key in res) {
              if (key === 'db_success') continue;
              const $field = $(`input[name="${key}"], select[name="${key}"]`);
              $field.addClass('is-invalid').next('.invalid-feedback').text(res[key]);
            }
            if (res.db_other || res.env_other) {
              layer.msg(res.db_other || res.env_other);
            }
          }
        },
        error: (xhr) => {
          layer.msg(xhr.responseJSON?.message || '{{ __('install/common.check_system') }}');
        }
      });
    },
    ...
    checkComplete(data, callback) {
      const loadingIndex = layer.load(2, {shade: [0.3, '#fff']});
      layer.msg('正在安装数据库与初始数据，约需 30–60 秒，请勿关闭页面', {time: 0});
      $('.is-invalid').removeClass('is-invalid').next('.invalid-feedback').text('');
      ...
      $.ajax({
        ...
        error: (xhr) => {
          ...
          layer.msg(body.message || `请求失败（HTTP ${xhr.status}），请查看服务器日志`);
        }
      });
    },
