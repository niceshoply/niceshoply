...
use NiceShoply\Console\Domain\Theme;
...
class ThemeService extends BaseService
{
    ...
    protected function validateConfig(array $config): void
    {
        ...
        foreach ($required as $key) {
            if (! isset($config[$key])) {
                throw new Exception(__('console/themes.error_missing_field', ['field' => $key]));
            }
        }
    }
    ...
    protected function validateCode(array $config, string $folderName): void
    {
        if (! isset($config['code']) || strtolower($config['code']) !== $folderName) {
            throw new Exception(__('console/themes.error_code_mismatch', [
                'folder' => $folderName,
                'code'   => $config['code'],
            ]));
        }
        ...
        if ($config['code'] !== strtolower($config['code'])) {
            throw new Exception(__('console/themes.error_code_not_lowercase', [
                'code' => $config['code'],
            ]));
        }
    }
     ...
     * @throws Exception
     */
    public function runDemoSeeder(string $dir, bool $clearCatalog = false): void
    {
        ThemeDemoService::getInstance()->importDemo($dir, $clearCatalog);
    }
