...
class ThemeDemoService extends BaseService
{
     ...
     *
     * @param  string  $dir  主题目录
     * @param  bool  $clearCatalog  导入前是否安全清空现有店铺目录数据（IMP-09，需用户显式确认）
     * @throws Exception
     */
    public function importDemo(string $dir, bool $clearCatalog = false): void
    {
        // IMP-09：可选的安全清库，避免外键冲突导致导入失败（仅在用户显式勾选时执行）
        if ($clearCatalog) {
    ...
    {
        if (! file_exists($file)) {
            throw new Exception(__('console/themes.error_demo_sql_not_found', ['file' => basename($file)]));
        }

        if (! is_readable($file)) {
            throw new Exception(__('console/themes.error_demo_sql_not_readable', ['file' => basename($file)]));
        }

        $sql = file_get_contents($file);
        if ($sql === false || trim($sql) === '') {
            throw new Exception(__('console/themes.error_demo_sql_empty'));
        }
                ...
                if (strpos($errorMessage, 'no active transaction') !== false) {
                    ...
                    if ($inTransaction && $transactionLevelAfter === 0) {
                        try {
                            ...
                            throw new Exception(__('console/themes.error_demo_sql_execute_failed', [
                                'file'  => basename($file),
        ...
        if ($queryCount === 0) {
            throw new Exception(__('console/themes.error_demo_sql_no_queries', ['file' => basename($file)]));
        }
    }
        ...
        foreach ($images as $image) {
            ...
            if (! is_dir($targetDir)) {
                if (! mkdir($targetDir, 0755, true)) {
                    throw new Exception(__('console/themes.error_demo_image_dir_failed', [
                        'dir' => $targetDir,
                    ]));
                }
            }
            ...
            if (! copy($image, $targetPath)) {
                throw new Exception(__('console/themes.error_demo_image_copy_failed', [
                    'file' => basename($image),
                ]));
            }
        }
