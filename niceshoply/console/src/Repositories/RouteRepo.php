...
class RouteRepo
{
    const IGNORE_LIST = [
        ...
        'system_update.progress', 'system_update.log', 'backups.progress',
    ];
     ...
     * @return array
     */
    public function getConsolePermissions(): array
    {
        $permissions   = [];
        $consoleRoutes = $this->getConsoleRoutes();
        foreach ($consoleRoutes as $module => $routes) {
            $isSystem = in_array($module, $this->systemModules);
            if ($isSystem) {
                $label = console_trans("menu.$module");
            } else {
     ...
     * @return array
     */
    public function getConsoleRoutes(): array
    {
        $routeList = [];
        $routes    = Route::getRoutes();
        foreach ($routes as $route) {
            $routeName = $route->getName();
            if (! Str::startsWith($route->getName(), 'console.')) {
                continue;
            }

            $routeName = substr($routeName, strlen('console.'));
    ...
    {
        if (Str::startsWith($controller, 'NiceShoply\\Console')
            || Str::startsWith($controller, 'NiceShoply\\Plugin')) {
            return '';
        }
    ...
    private function getPermissionList($routes): array
    {
        ...
        foreach ($routes as $route) {
            ...
            if ($pluginSpace) {
                ...
                $label = console_trans("route.{$routeSlug}");
            }
