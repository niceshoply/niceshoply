...
@section('title', console_trans('options.option_management'))
  ...
  <div class="card h-min-600" id="app">
    <div class="card-body">
      ...
      <ul class="nav nav-tabs mb-3">
        <li class="nav-item">
          <a class="nav-link active" href="{{ console_route('options.index') }}">
            <i class="bi bi-collection"></i> {{ console_trans('options.option_group_management') }}
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="{{ console_route('option_values.index') }}">
            <i class="bi bi-list-ul"></i> {{ console_trans('options.option_value_management') }}
          </a>
        </li>
      </ul>

      <x-console-data-criteria :criteria="$criteria ?? []" :action="console_route('options.index')" />
      ...
      @if ($option_groups->count())
        <div class="table-responsive">
          <table class="table align-middle">
            ...
            <tr>
              ...
              <th>{{ console_trans('options.description') }}</th>
              <th>{{ console_trans('options.type') }}</th>
              <th>{{ console_trans('options.is_required') }}</th>
              <th>{{ console_trans('options.sort') }}</th>
              ...
              <tr>
                ...
                <td>
                  <div class="text-muted small" style="max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" 
                    ...
                    {{ $optionGroup->getLocalizedDescription() ?: console_trans('options.no_description') }}
                  </div>
                </td>
                <td>
                  @switch($optionGroup->type)
                    @case('select')
                      <span class="badge bg-primary">{{ console_trans('options.dropdown_select') }}</span>
                      @break
                    @case('radio')
                      <span class="badge bg-info">{{ console_trans('options.radio_button') }}</span>
                      @break
                    @case('checkbox')
                      <span class="badge bg-success">{{ console_trans('options.checkbox') }}</span>
                      @break
                    @case('text')
                      <span class="badge bg-warning">{{ console_trans('options.text_input') }}</span>
                      @break
                    @case('textarea')
                      <span class="badge bg-secondary">{{ console_trans('options.textarea') }}</span>
                      @break
                    @default
                ...
                <td>
                  @if($optionGroup->required)
                    <span class="badge bg-danger">{{ console_trans('options.required') }}</span>
                  @else
                    <span class="badge bg-secondary">{{ console_trans('options.optional') }}</span>
                  @endif
                </td>
                <td>{{ $optionGroup->position }}</td>
                <td>
                  @include('console::shared.list_switch', [
                    'value' => $optionGroup->active, 
                    'url' => console_route('options.active', $optionGroup->id)
                  ])
                </td>
                  ...
                  <div class="d-flex gap-2">
                    <div>
                      <button type="button" class="btn btn-sm btn-outline-primary" 
                        ...
                        {{ __('console/common.edit') }}
                      </button>
                    </div>
                    <div>
                      <form action="{{ console_route('options.destroy', [$optionGroup->id]) }}"
                            method="POST" class="d-inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-sm btn-outline-danger" 
                                onclick="return confirm('{{ console_trans('options.confirm_delete_option_group') }}')">
                          {{ __('console/common.delete') }}
                        </button>
                      </form>
                    </div>
                  </div>
        ...
        {{ $option_groups->withQueryString()->links('console::vendor/pagination/bootstrap-4') }}
      @else
        <x-common-no-data />
      @endif
    </div>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="optionGroupModalLabel">{{ console_trans('options.create_option_group') }}</h5>
        ...
        <form id="optionGroupForm" method="POST">
          ...
          <div class="modal-body">
            <div class="row">
              <!-- Basic information -->
              <div class="col-12">
                <h6 class="mb-3">{{ console_trans('options.basic_info') }}</h6>
              </div>
              
              <div class="col-12 col-md-6">
                <div class="mb-3">
                  <label for="type" class="form-label">{{ console_trans('options.option_type') }} <span class="text-danger">*</span></label>
                  <select class="form-select" id="type" name="type" required>
                    <option value="select" selected>{{ console_trans('options.dropdown_select') }}</option>
                    <option value="radio">{{ console_trans('options.radio_button') }}</option>
                    <option value="checkbox">{{ console_trans('options.checkbox') }}</option>
                  </select>
                </div>
              </div>
              ...
              <div class="col-12 col-md-6">
                <div class="mb-3">
                  <label for="position" class="form-label">{{ console_trans('options.sort') }}</label>
              ...
              <div class="col-12 col-md-6">
                <div class="mb-3">
                  <label class="form-label">{{ console_trans('options.is_required') }}</label>
              ...
              <div class="col-12 col-md-6">
                <div class="mb-3">
                  <label class="form-label">{{ console_trans('options.is_enabled') }}</label>
                  <div class="form-check form-switch">
              ...
              <div class="col-12">
                <h6 class="mb-3 mt-3">{{ console_trans('options.multilingual_info') }}</h6>
                  ...
                  @foreach (locales() as $locale)
                    <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" 
                      ...
                      <div class="row">
                        <div class="col-12">
                          <div class="mb-3">
                            <label for="name_{{ $locale['code'] }}" class="form-label">
                              {{ console_trans('options.option_group_name') }}
                        ...
                        <div class="col-12">
                          <div class="mb-3">
                            <label for="description_{{ $locale['code'] }}" class="form-label">{{ console_trans('options.option_group_description') }}</label>
                            <textarea class="form-control" 
                                      id="description_{{ $locale['code'] }}" 
                                      name="translations[{{ $locale['code'] }}][description]"
                                      rows="3" 
                                      placeholder="{{ console_trans('options.option_group_description_placeholder') }}"></textarea>
                          </div>
                        </div>
                      </div>
                    </div>
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ console_trans('options.cancel') }}</button>
            <button type="submit" class="btn btn-primary">{{ console_trans('options.save') }}</button>
          </div>
        </form>
      </div>
    </div>
...
$(document).ready(function() {
    ...
    window.openCreateModal = function() {
        ...
        $('#optionGroupModalLabel').text('{{ console_trans('options.create_option_group') }}');
        $('#optionGroupForm').attr('action', '{{ console_route("options.store") }}');
    ...
    window.openEditModal = function(optionId) {
        ...
        $('#optionGroupForm').attr('action', `/console/options/${optionId}`);
        ...
        $('#optionGroupModalLabel').text('{{ console_trans('options.edit_option_group') }}');
        // Show modal
        $('#optionGroupModal').modal('show');
    };
    ...
    $('#optionGroupForm').on('submit', function(e) {
        ...
        submitBtn.prop('disabled', true).text('{{ console_trans('options.processing') }}');
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                layer.msg(response.message || '{{ console_trans('options.operation_success') }}', {
                    icon: 1,
                    time: 2000
                }, function() {
                    // Refresh page
            ...
            error: function(xhr) {
                ...
                let errorMessage = '{{ console_trans('options.operation_failed') }}';
