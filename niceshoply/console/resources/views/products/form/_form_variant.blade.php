...
<div class="card variants-box mb-3" id="variants-box">
  <div class="card-header">
    <h5 class="card-title mb-0">{{ __('console/product.variant') }}</h5>
  </div>
  ...
  <div class="card-body py-0">
              ...
              <div class="variant-header d-flex justify-content-between align-items-center mb-2">
                <div class="d-flex align-items-center">
                  ...
                  <label class="form-check-label ms-3">
                    ...
                    {{ __('console/product.is_image_variant') }}
                  </label>
                </div>
                <div class="action-buttons">
                  <button type="button" class="btn btn-outline-primary btn-sm" @click="openVariantDialog(index, null)">{{ __('console/common.edit') }}</button>
                  <button type="button" class="btn btn-outline-danger btn-sm ms-2" @click="deleteVariant(index)">{{ __('console/common.delete') }}</button>
                </div>
              </div>
                ...
                <div class="variant-values-container">
                  ...
                  <div class="mt-2">
                    <small class="text-muted">
                      ...
                      {{ __('console/product.variant_value_edit_tip') }}
                    </small>
                  </div>
                </div>
              </div>
            ...
            <div class="add-variant-form" v-else>
              <div class="mb-3 add-variant-title">
                <div class="variant-label">
                  <label class="form-label">{{ __('console/product.variant_name') }}</label>
                  <div class="v-locales-input">
                    <div v-for="locale in locales" class="input-group" :key="locale.code">
                      ...
                      <input type="text" class="form-control" v-model="variant.name[locale.code]" placeholder="{{ __('console/product.variant_name_help') }}">
                    </div>
                    <span class="text-12 text-danger" style="margin-left: 100px" v-if="variant.error"><i class="bi bi-exclamation-circle"></i> {{ __('console/common.verify_required') }}</span>
                  </div>
                    @hookinsert('console.product.edit.variant_name.after')
                </div>
              </div>
              <div class="add-variant-values">
                <label class="form-label">{{ __('console/product.variant_value') }}</label>
                <div class="add-variant-value">
                  <div class="add-variant-value-item" v-for="(value, index) in variant.values" :key="index">
                    ...
                    <div class="v-locales-input variant-value">
                      <div v-for="locale in locales" class="input-group" :key="locale.code">
                        <span class="input-group-text"><img :src="'/images/flag/'+ locale.code +'.png'" class="img-fluid">@{{ locale.name }}</span>
                        <input type="text" class="form-control" v-model="value.name[locale.code]" placeholder="{{ __('console/product.variant_value_help') }}" ref="variantValue">
                      </div>
                      <span class="text-12 text-danger" style="margin-left: 100px" v-if="value.error"><i class="bi bi-exclamation-circle"></i> {{ __('console/common.verify_required') }}</span>
                    </div>
                  ...
                  <div class="add-variant-btns">
                    <div class="text-primary text-12 mb-3">
                      <div class="d-inline-block cursor-pointer" @click="addVariantValue(index)"><i class="bi bi-plus-lg"></i> {{ __('console/product.add_variant_value') }}</div>
                    </div>
                    <div class="d-flex align-items-center justify-content-between">
                      <button type="button" class="btn btn-outline-danger" @click="deleteVariant(index)">{{ __('console/common.delete') }}</button>
                      <button type="button" class="btn btn-outline-primary" @click="saveVariant(index)">{{ __('console/common.btn_save') }}</button>
                    </div>
                  </div>
                </div>
              </div>
    ...
    <div :class="['text-primary add-variant', !variants.length ? 'no-variants' : '']">
      <div class="d-inline-block cursor-pointer" @click="openVariantDialog(-1, null)"><i class="bi bi-plus-square me-1"></i> {{ __('console/product.add_variant') }}</div>
    </div>
    <div class="variant-skus-wrap" v-if="smallVariants.length">
      <div class="batch-settings-console mb-3">
        <div class="card shadow-sm" style="border: none;">
          <div class="card-body py-3">
            <div class="mb-2" v-if="variants.length > 0">
              <label class="form-label small fw-bold mb-2">{{ __('console/product.sku_batch_setting') }}</label>
                  ...
                  <div class="col-md-10">
                    <div class="d-flex flex-wrap gap-1 align-items-center">
                      ...
                      <button type="button" class="btn btn-outline-primary btn-sm ms-2" 
                        ...
                        {{ __('console/product.select_all') }}
                      </button>
                      <button type="button" class="btn btn-outline-secondary btn-sm" 
                              @click="clearVariantSelection(vIndex)">
                        {{ __('console/product.clear') }}
                      </button>
                    </div>
                  </div>
                </div>
            ...
            <div class="row g-2">
              <!-- SKU编码前缀 -->
              <div class="col-md-2">
                <label class="form-label small mb-1">SKU {{ __('console/product.bulk_fill') }}</label>
                <input type="text" class="form-control form-control-sm" v-model="batchData.skuPrefix" 
                       placeholder="{{ __('console/product.bulk_fill_sku') }}" style="height: 31px;">
              </div>
              
              <!-- 价格 -->
              <div class="col-md-2">
                <label class="form-label small mb-1">{{ __('console/product.price') }}</label>
                <input type="number" class="form-control form-control-sm" v-model="batchData.price" 
                       placeholder="{{ __('console/product.bulk_fill_price') }}" min="0" @input="validateBatchPrice" style="height: 31px;">
              </div>
              
              <!-- 原价 -->
              <div class="col-md-2">
                <label class="form-label small mb-1">{{ __('console/product.origin_price') }}</label>
                <input type="number" class="form-control form-control-sm" v-model="batchData.originPrice" 
                       placeholder="{{ __('console/product.bulk_fill_origin_price') }}" min="0" @input="validateBatchOriginPrice" style="height: 31px;">
              </div>
              
              <!-- 型号 -->
              <div class="col-md-2">
                <label class="form-label small mb-1">{{ __('console/product.model') }}</label>
                <input type="text" class="form-control form-control-sm" v-model="batchData.model" 
                       placeholder="{{ __('console/product.bulk_fill_model') }}" style="height: 31px;">
              </div>
              
              <!-- 数量 -->
              <div class="col-md-2">
                <label class="form-label small mb-1">{{ __('console/product.quantity') }}</label>
                <input type="number" class="form-control form-control-sm" v-model="batchData.quantity" 
                       placeholder="{{ __('console/product.bulk_fill_quantity') }}" min="0" @input="validateBatchQuantity" style="height: 31px;">
              </div>
              
              <!-- SKU图片 -->
              <div class="col-md-2">
                <label class="form-label small mb-1">{{ __('console/product.sku_image') }}</label>
                <div class="d-flex align-items-center" style="height: 31px;">
                  ...
                  <button type="button" class="btn btn-outline-secondary btn-sm" @click="selectBatchImage" style="font-size: 11px; padding: 2px 8px;">
                    <i class="bi bi-image me-1"></i>{{ __('console/product.select_image') }}
                  </button>
              ...
              <div class="col-md-2">
                ...
                <button type="button" class="btn btn-success w-100 fw-bold" @click="batchApplySelected" style="height: 31px; font-size: 12px;">
                  <i class="bi bi-lightning-charge-fill me-1"></i>{{ __('console/product.bulk_fill') }}
                </button>
              </div>
              
              @hookinsert('console.product.edit.sku.batch.input.item.after')
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="variant-skus-table table-responsive">
        <table class="table align-middle table-bordered">
          <thead class="table-light">
            <tr>
              <th style="min-width: 220px">{{ __('console/product.variant') }}</th>
              <th>SKU Code</th>
              <th>{{ __('console/product.price') }}</th>
              <th>{{ __('console/product.origin_price') }}</th>
              <th>{{ __('console/product.model') }}</th>
          ...
          <tbody>
            <tr v-for="(sku, index) in skus" :key="index">
                  ...
                  <div>
                    ...
                    <div class="up-master text-12">
                      <span v-if="sku.is_default" class="text-success">
                        <i class="bi bi-check-circle-fill"></i> {{ __('console/product.main_variant') }}
                      </span>
                      <span class="opacity-50 cursor-pointer" v-else @click="setMasterSku(index)">
                        <i class="bi bi-circle"></i> {{ __('console/product.main_variant') }}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <td>
                ...
                <div class="invalid-feedback">{{ __('console/product.error_sku_repeat') }}</div>
              </td>
              <td>
                <input type="text" class="form-control form-control-sm"
                  v-model="sku.price" placeholder="{{ __('console/product.price') }}"
                  @input="validatePrice(sku)">
                  @hookinsert('console.product.edit.sku.input.item.price.after')
              </td>
              <td>
                <input type="text" class="form-control form-control-sm"
                  v-model="sku.origin_price" placeholder="{{ __('console/product.origin_price') }}"
                  @input="validateOriginPrice(sku)">
              </td>
              <td>
                <input type="text" class="form-control form-control-sm"
                  v-model="sku.model" placeholder="{{ __('console/product.model') }}">
              </td>
              <td>
                <input type="text" class="form-control form-control-sm"
                  v-model="sku.quantity" placeholder="{{ __('console/product.quantity') }}"
    ...
    @hookinsert('console.product.edit.variant.after')
      ...
      <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
              ...
              <div class="mb-3">
                <div v-for="locale in locales" :key="locale.code" class="input-group mb-2">
                  ...
                  <input type="text" class="form-control" 
                         ...
                         :placeholder="'{{ __('console/product.name') }}'"
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" @click="closeVariantDialog">{{ __('console/common.cancel') }}</button>
            <button type="button" class="btn btn-primary" @click="saveVariantDialog">{{ __('console/common.save') }}</button>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  let variantsBoxApp = createApp({
    ...
    setup() {
      ...
      const defaultLocale = @json(console_locale_code());
      ...
      const openVariantDialog = (variantIndex, valueIndex = null) => {
        ...
        if (variantIndex === -1) {
          ...
          name = localesFill('');
          title = '{{ __('console/product.add_variant') }}';
          ...
          title = '{{ __('console/product.edit_variant') }}';
        } else if (valueIndex === -1) {
          // Creating new variant value
          name = localesFill('');
          title = '{{ __('console/product.add_variant_value') }}';
        } else {
          ...
          title = '{{ __('console/product.edit_variant_value') }}';
        }
      ...
      const saveVariantDialog = () => {
        ...
        if (isObjectValuesEmpty(name)) {
          layer.msg('{{ __('console/common.verify_required') }}', {icon: 2});
          return;
        }
        ...
        closeVariantDialog();
        layer.msg('{{ __('console/common.saved_success') }}', {icon: 1});
      };
      ...
      const deleteVariantValue = (variantIndex, valueIndex) => {
        if (confirm('{{ __('console/common.confirm_delete') }}')) {
          variants.value[variantIndex].values.splice(valueIndex, 1);
          layer.msg('{{ __('console/common.deleted_success') }}', {icon: 1});
        }
      };
