<div class="mb-3 col-12 col-md-8" id="product-video">
  <label class="form-label">{{ __('console/product.video') }}</label>
    ...
    <div class="tab-content p-3" id="nav-tabContent">
        ...
        <div class="d-flex align-items-end">
          ...
          <div v-if="videoForm.url" class="video-actions">
            <a target="_blank" :href="videoForm.url" class="btn btn-sm btn-outline-primary mb-2">
              <i class="bi bi-eye me-1"></i>{{ __('console/common.preview') }}
            </a>
            <button type="button" @click="deleteVideo" class="btn btn-sm btn-outline-danger">
              <i class="bi bi-trash me-1"></i>{{ __('console/common.delete') }}
            </button>
          </div>
        </div>
        <div class="form-text mt-2">
          <i class="bi bi-info-circle me-1"></i>{{ __('console/product.video_local_help') }}
        </div>
      </div>
      ...
      <div :class="['tab-pane fade', videoForm.videoType == 'iframe' ? 'show active' : '']" id="nav-v-iframe">
        <textarea class="form-control" 
                  rows="3" 
                  placeholder="{{ __('console/product.video_iframe_placeholder') }}" 
                  v-model="videoForm.iframe"></textarea>
        <div class="form-text mt-2">
          <i class="bi bi-info-circle me-1"></i>{{ __('console/product.video_iframe_help') }}
        </div>
      </div>
      
      <div :class="['tab-pane fade', videoForm.videoType == 'custom' ? 'show active' : '']" id="nav-v-custom">
        <input class="form-control" 
               placeholder="{{ __('console/product.video_custom_placeholder') }}" 
               v-model="videoForm.custom">
        <div class="form-text mt-2">
          <i class="bi bi-info-circle me-1"></i>{{ __('console/product.video_custom_help') }}
        </div>
      </div>
    </div>
