    ...
    <div class="col-12">
      <div class="card" id="product-options-app">
        <div class="card-header">
          <h5 class="card-title mb-0">{{ __('console/product.product_options') }}</h5>
          <small class="text-muted">{{ __('console/product.product_options_description') }}</small>
        </div>
        <div class="card-body">
          ...
          <div class="row">
            ...
            <div class="col-md-6">
              <div class="border rounded p-3 h-100">
                <h6 class="mb-3 d-flex justify-content-between align-items-center">
                  <span>
                    <i class="bi bi-list-ul me-2"></i>{{ __('console/product.available_options') }}
                    <span class="badge bg-secondary ms-2">@{{ availableOptionsFiltered.length }}</span>
                  </span>
                  <a href="{{ console_route('options.index') }}" class="btn btn-sm btn-outline-primary" target="_blank" title="{{ __('console/options.option_management') }}">
                    <i class="bi bi-gear me-1"></i>{{ __('console/options.option_management') }}
                  </a>
                </h6>
                ...
                <div class="mb-3">
                  <div class="input-group">
                    ...
                    <input type="text" class="form-control" v-model="searchTerm" 
                           placeholder="{{ __('console/product.search_placeholder_options') }}">
                  </div>
                </div>
                ...
                <div style="max-height: 500px; overflow-y: auto;">
                  ...
                  <div v-else-if="availableOptionsFiltered.length === 0" class="text-center py-4 text-muted">
                    ...
                    <p class="mb-0">{{ __('console/product.no_options_available') }}</p>
                  </div>
                          ...
                          <div class="d-flex align-items-center">
                            ...
                            <div class="flex-grow-1">
                              ...
                              <div class="text-muted small">
                                @{{ option.type }} • @{{ option.option_values_count || 0 }} {{ __('console/product.option_values_count') }}
                              </div>
                            </div>
                          </div>
                        </div>
            ...
            <div class="col-md-6">
              <div class="border rounded p-3 h-100">
                <h6 class="mb-3">
                  <i class="bi bi-check-square me-2"></i>{{ __('console/product.configured_options') }}
                ...
                <div style="max-height: 500px; overflow-y: auto;">
                  <div v-if="selectedOptions.length === 0" class="text-center py-4 text-muted">
                    <i class="bi bi-inbox display-6 d-block mb-2"></i>
                    <p class="mb-0">{{ __('console/common.no_data') }}</p>
                    <small>{{ __('console/product.select_option') }}</small>
                  </div>
                              ...
                              <table class="table table-sm mb-0">
                                <thead>
                                  <tr>
                                    ...
                                    <th>{{ __('console/product.option_values') }}</th>
                                    <th width="120">{{ __('console/product.price_adjustment') }}</th>
                                    <th width="100">{{ __('console/product.stock_quantity') }}</th>
                                  </tr>
                                </thead>
                                <tbody>
...
createOptionsApp({
  setup() {
    ...
    const loadAvailableOptions = async () => {
      loading.value = true;
      try {
        const response = await fetch(urls.console_base + '/options/available', {
          method: 'GET',
          headers: {
        ...
        if (data.success) {
          ...
          showError('{{ __("console/product.load_options_error") }}');
        }
      } catch (error) {
    ...
    const loadOptionValues = async (option) => {
      try {
        const response = await fetch(urls.console_base + '/options/' + option.id + '/values', {
          method: 'GET',
          headers: {
            'X-Requested-With': 'XMLHttpRequest',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
