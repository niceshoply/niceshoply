<div class="mt-3">
  <label class="form-label">{{ __('console/product.bundle_items') }}</label>
      ...
      <div class="bundle-item d-flex align-items-center mb-2">
        <div class="input-group flex-grow-1">
          <input type="text" class="form-control sku-autocomplete" 
                   ...
                   placeholder="{{ __('console/product.bundle_sku_placeholder') }}" 
        ...
        <button type="button" class="btn btn-danger btn-sm remove-bundle-item">{{ __('console/common.delete') }}</button>
      </div>
    @endforeach
  </div>
  <button type="button" class="btn btn-primary btn-sm mt-2" id="add-bundle-item">{{ __('console/product.add_bundle_item') }}</button>
</div>
...
$(function () {
  function initSkuAutocomplete() {
    $('.sku-autocomplete').autocomplete({
      source: function(request, response) {
        ...
        let url = '/api/v1/products/skus?keyword=' + keyword + '&limit=10';
  ...
  $('#add-bundle-item').on('click', function() {
    ...
    const html = `
      <div class="bundle-item d-flex align-items-center mb-2">
        <div class="input-group flex-grow-1">
          <input type="text" class="form-control sku-autocomplete" name="bundles[${index}][sku_code_text]" placeholder="{{ __('console/product.bundle_sku_placeholder') }}" autocomplete="off" required>
