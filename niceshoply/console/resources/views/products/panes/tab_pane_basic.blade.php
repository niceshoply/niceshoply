<div class="tab-pane fade show active mt-3" id="basic-tab-pane" role="tabpanel" aria-labelledby="basic-tab"
  ...
  <div class="mb-3 col-12 col-md-6">
    <label class="form-label">{{ __('console/product.type') }}</label>
    <select class="form-select" name="type" id="product-type" {{ $product->id ? 'disabled' : '' }} required>
      @php
        $productTypes = \NiceShoply\Common\Repositories\ProductRepo::getProductTypes();
    ...
    <div class="mt-2 text-muted small">
      <i class="bi bi-info-circle me-1"></i>{{ __('console/product.type_hint') }}
    </div>
  </div>

  <div class="mb-3 col-12 col-md-6">
    <div class="mb-1 fs-6">{{ __('console/product.name') }}</div>
    @if(has_translator())
      <div
        ...
        <div class="d-flex align-items-center me-3">{{ __('console/product.auto_translate') }}</div>
    ...
    @foreach (locales() as $locale)
      ...
      <div class="input-group mb-2">
        ...
        <input type="text" class="form-control" name="translations[{{ $localeCode }}][name]"
               ...
               required placeholder="{{ __('console/product.name') }}" aria-label="{{ $localeName }}"
               aria-describedby="basic-addon1" data-locale="{{ $localeCode }}">
      </div>
    @endforeach
    <div class="mt-1 text-muted small">
      <i class="bi bi-info-circle me-1"></i>{{ __('console/product.name_required_hint') }}
    </div>
  </div>

  <x-common-form-images title="{{ __('console/common.image') }}" name="images"
                        :values="old('images', $product->images ?? [])"/>

  <x-common-form-image title="{{ __('console/product.hover_image') }}" name="hover_image"
                       :value="old('hover_image', $product->hover_image ?? '')"
                       :description="__('console/product.hover_image_help')"/>

  @include('console::products.form._form_video')
    ...
    <div class="col-12">
      <div class="mb-3" id="spec-type-group">
        <div class="form-check form-check-inline">
          ...
          <label class="form-check-label" for="price_type_single">
            {{ __('console/product.price_type_single') }}
          </label>
        </div>
        <div class="form-check form-check-inline">
          ...
          <label class="form-check-label" for="price_type_multiple">
            {{ __('console/product.price_type_multiple') }}
          </label>
        </div>
      </div>
      ...
      <div id="single_price_box" class="{{ $product->isMultiple() ? 'd-none' : '' }}">
        <div class="row">
          <div class="col-12 col-md-3">
            <x-common-form-input :title="__('console/product.price')" name="skus[0][price]"
                               value="{{ old('skus.0.price', $product->masterSku->price ?? '') }}" required/>
          </div>
          <div class="col-12 col-md-3">
            <x-common-form-input :title="__('console/product.quantity')" name="skus[0][quantity]"
                               value="{{ old('skus.0.quantity', $product->masterSku->quantity ?? '') }}" required/>
          </div>
          <div class="col-12 col-md-3">
            <x-common-form-input :title="__('console/product.sku_code')" name="skus[0][code]"
                               value="{{ old('skus.0.code', $product->masterSku->code ?? '') }}" required/>
          </div>
          <div class="col-12 col-md-3">
            <x-common-form-input :title="__('console/product.origin_price')" name="skus[0][origin_price]"
      ...
      <div id="specifications_box" class="{{ !$product->isMultiple() ? 'd-none' : '' }}">
        <div class="alert alert-info mb-3" id="multi_spec_notice">
          ...
          {{ __('console/product.multiple_spec_notice') }}
        </div>
        @include('console::products.form._form_variant')
        @hookinsert('console.product.edit.form_variant.after')
      </div>
    </div>
  </div>

  <x-common-form-switch-radio :title="__('console/common.status')" name="active"
                              :value="old('active', $product->active ?? true)"/>
  @hookinsert('console.product.edit.basic.after')
</div>
