<div class="tab-pane fade mt-3" id="extra-tab-pane" role="tabpanel" aria-labelledby="extra-tab"
  ...
  <div class="row">
    <div class="col-12 col-md-6" id="product-category">
      {{-- 分类选择 --}}
      <x-console::form.row title="{{ __('console/product.category') }}">
    ...
    <div class="col-12 col-md-6">
      <x-common-form-select :title="__('console/product.brand')" name="brand_id"
  ...
  <div class="row">
    <div class="col-12 col-md-6">
      <div class="row">
        <div class="col-md-6">
          <x-common-form-input :title="__('console/product.weight')" name="weight"
                           :value="old('weight', $product->weight ?? '')" :placeholder="__('console/product.weight')"/>
        </div>
        <div class="col-md-6">
          <x-common-form-select :title="__('console/product.weight_class')" name="weight_class"
                             :value="old('weight_class', $product->weight_class ?? '')" :options="$weightClasses"
                             key="code" label="name" />
        </div>
      </div>
      <x-common-form-select :title="__('console/product.tax_class')" name="tax_class_id"
                            :value="old('tax_class_id', $product->tax_class_id ?? 0)" :options="$tax_classes"
                            key="id" label="name"/>
    </div>
    <div class="col-12 col-md-6">
      <x-common-form-input :title="__('console/product.spu_code')" name="spu_code"
                           :value="old('spu_code', $product->spu_code ?? '')"
                           :placeholder="__('console/product.spu_code')"/>
  ...
  <div class="row">
    <div class="col-12 col-md-6">
      <x-common-form-input :title="__('console/product.sales')" name="sales" :value="old('sales', $product->sales ?? '')"
                           :placeholder="__('console/product.sales')"/>
      <x-common-form-input :title="__('console/product.viewed')" name="viewed"
                           :value="old('viewed', $product->viewed ?? '')" :placeholder="__('console/product.viewed')"/>
    </div>
    <div class="col-12 col-md-6">
      <x-common-form-switch-radio :title="__('console/product.is_virtual')" name="is_virtual"
