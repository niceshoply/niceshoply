  ...
  <div class="card h-min-600">
    ...
    <div class="card-body">
      <form class="needs-validation" novalidate id="app-form"
            action="{{ $transaction->id ? console_route('transactions.update', [$transaction->id]) : console_route('transactions.store') }}"
            method="POST">
        @csrf
        ...
        <div class="row">
          <div class="col-12 col-md-6">
            <div class="col-sm-2 required col-form-label text-start client d-flex">
              ...
              <div>{{ __('console/transaction.customer') }}</div>
            </div>
            <input type="text" name="customer_name" id="customer-autocomplete"
                   value="{{ old('customer_name', $transaction->customer->name ?? '') }}" placeholder="{{ __('console/transaction.customer_search') }}"
                   class="form-control">
          ...
          <div class="col-12 col-md-6">
            <x-common-form-input title="{{ __('console/transaction.amount') }}" name="amount"
                                 value="{{ old('amount', $transaction->amount) }}" required/>
          </div>

          <div class="col-12 col-md-6">
            <x-common-form-select :title="__('console/transaction.type')" name="type" :emptyOption="false" required
                                  :value="old('type', $transaction->type)" :options="$types" key="code"
                                  label="label"/>
          </div>

          <div class="col-12 col-md-6">
            <x-common-form-textarea title="{{ __('console/transaction.comment') }}" name="comment"
