...
@section('title', __('console/menu.states'))

@section('content')
  <div class="card h-min-600" id="app">
    <div class="card-body">

      <x-console-data-criteria :criteria="$criteria ?? []" :action="console_route('states.index')"/>
      ...
      @if ($states)
        <div class="table-responsive">
          <table class="table align-middle">
            ...
            <tr>
              ...
              <th>{{ __('console/state.name') }}</th>
              <th>{{ __('console/state.code') }}</th>
              <th>{{ __('console/state.country_code') }}</th>
              <th>{{ __('console/state.position') }}</th>
              <th>{{ __('console/state.active') }}</th>
            ...
            @foreach($states as $item)
              <tr>
                ...
                <td>{{ $item['position'] }}</td>
                <td>
                  @include('console::shared.list_switch', ['value' => $item->active, 'url' => console_route('states.active',
                  $item->id)])
                </td>
                <td>
                  <div class="d-flex gap-1">
                    <el-button size="small" @click="edit({{ $item->id }})" plain
                               type="primary">{{ __('console/common.edit')}}
                    </el-button>
                    <form ref="deleteForm" action="{{ console_route('states.destroy', [$item->id]) }}" method="POST"
                          class="d-inline">
                      @csrf
                      @method('DELETE')
        ...
        {{ $states->withQueryString()->links('console::vendor/pagination/bootstrap-4') }}
      @else
        <x-common-no-data/>
      @endif
    </div>
    ...
    <el-drawer v-model="drawer" size="500" @close="close">
      <template #header>
        <div class="text-dark fs-4">{{ __('console/menu.states') }}</div>
      </template>
...
@push('footer')
  <script>
    const api = @json(console_route('states.index'));
      ...
      setup() {
        ...
        const open = (itemId) => {
          ElMessageBox.confirm(
            ...
            .then(() => {
              const deletUrl = urls.console_base + '/states/' + itemId;
