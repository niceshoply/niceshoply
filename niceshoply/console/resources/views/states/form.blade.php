...
@section('title', __('console/menu.state'))

@section('content')
<div class="card h-min-600">
  <div class="card-header">
    <h5 class="card-title mb-0">{{ __('console/menu.state') }}</h5>
  </div>
  <div class="card-body">
    <form class="needs-validation mt-3" novalidate action="{{ $state->id ? console_route('states.update', [$state->id]) : console_route('states.store') }}" method="POST">
      @csrf
