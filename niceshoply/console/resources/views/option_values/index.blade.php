...
@section('title', console_trans('options.option_value_management'))
  ...
  <div class="card h-min-600" id="app">
    <div class="card-body">
      ...
      <ul class="nav nav-tabs mb-3">
        <li class="nav-item">
          <a class="nav-link" href="{{ console_route('options.index') }}">
            <i class="bi bi-collection"></i> {{ console_trans('options.option_group_management') }}
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link active" href="{{ console_route('option_values.index') }}">
            <i class="bi bi-list-ul"></i> {{ console_trans('options.option_value_management') }}
          </a>
        </li>
      </ul>

      <x-console-data-criteria :criteria="$criteria ?? []" :action="console_route('option_values.index')" />
      ...
      @if ($optionValues->count())
        <div class="table-responsive">
          <table class="table align-middle">
            ...
            <tr>
              ...
              <th>{{ console_trans('options.option_group') }}</th>
              <th>{{ console_trans('options.image') }}</th>
              ...
              <tr>
                ...
                <td>
                  @if($optionValue->image)
                    <img src="{{ $optionValue->getImageUrl() }}" alt="{{ console_trans('options.option_value_image') }}" class="img-thumbnail" style="width: 40px; height: 40px;">
                  @else
                    <span class="text-muted">-</span>
                  @endif
                </td>
                ...
                <td>
                  @if ($optionValue->active)
                    <span class="badge bg-success">{{ __('console/common.active') }}</span>
                  @else
                    <span class="badge bg-secondary">{{ __('console/common.inactive') }}</span>
                  @endif
                </td>
                <td>{{ $optionValue->created_at }}</td>
                <td>
                  <button type="button" class="btn btn-outline-primary btn-sm" onclick="openEditModal({{ $optionValue->id }})">
                    <i class="bi bi-pencil-square"></i> {{ __('console/common.edit') }}
                  </button>
                  <button type="button" class="btn btn-outline-danger btn-sm" onclick="if(confirm('{{ console_trans('options.confirm_delete_option_value') }}')) { document.getElementById('delete-form-{{ $optionValue->id }}').submit(); }">
                    <i class="bi bi-trash"></i> {{ __('console/common.delete') }}
                  </button>
                  <form id="delete-form-{{ $optionValue->id }}" action="{{ console_route('option_values.destroy', $optionValue) }}" method="POST" style="display: none;">
                    @csrf
                    @method('DELETE')
                  </form>
                </td>
        ...
        <div class="d-flex justify-content-center">
          {{ $optionValues->withQueryString()->links('console::vendor/pagination/bootstrap-4') }}
        </div>
      @else
        <x-common-no-data />
      @endif
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="optionValueModalLabel">{{ console_trans('options.add_option_value') }}</h5>
          ...
          <div class="modal-body">
            <div class="row">
              ...
              <div class="col-12 col-md-6">
                <div class="mb-3">
                  <label for="option_id" class="form-label">{{ console_trans('options.option_group') }} <span class="text-danger">*</span></label>
              ...
              <div class="col-12 col-md-6">
                <div class="mb-3">
                  <label for="image" class="form-label">{{ console_trans('options.image') }}</label>
                  ...
                  <div class="form-text">{{ console_trans('options.optional_option_value_image') }}</div>
                </div>
              </div>
              ...
              <div class="col-12 col-md-6">
                <div class="mb-3">
                  <label class="form-label">{{ console_trans('options.status') }}</label>
            ...
            <div class="mt-4">
              <h6 class="mb-3">{{ console_trans('options.multilingual_info') }}</h6>
                  ...
                  <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}" 
                    ...
                    <div class="mb-3">
                      <label for="name_{{ $locale['code'] }}" class="form-label">
                        {{ console_trans('options.option_name') }} ({{ $locale['name'] }})
...
<script>
  ...
  function openCreateModal() {
    ...
    // Set modal title
    $('#optionValueModalLabel').text('{{ console_trans('options.create_option_value') }}');
    
    // Set form action and method
    $('#optionValueForm').attr('action', '{{ console_route("option_values.store") }}');
  ...
  function openEditModal(optionValueId) {
    ...
    // Set modal title
    $('#optionValueModalLabel').text('{{ console_trans('options.edit_option_value') }}');
    
    // Set form action and method
    $('#optionValueForm').attr('action', '{{ console_route("option_values.update", ":id") }}'.replace(':id', optionValueId));
    ...
    $.get('{{ console_route("option_values.show", ":id") }}'.replace(':id', optionValueId))
      .done(function(data) {
      ...
      .fail(function() {
        layer.msg('{{ console_trans('options.get_option_value_data_failed') }}', {icon: 2});
  ...
  $('#optionValueForm').on('submit', function(e) {
    ...
    // Show loading state
    submitBtn.prop('disabled', true).text('{{ console_trans('options.saving') }}');
    ...
    $.ajax({
      ...
      success: function(response) {
        layer.msg(isEditMode ? '{{ console_trans('options.option_value_update_success') }}' : '{{ console_trans('options.option_value_create_success') }}', {icon: 1});
        $('#optionValueModal').modal('hide');
          ...
          <div class="modal fade" id="modal-show-img">
            <div class="modal-dialog modal-dialog-centered modal-lg">
              <div class="modal-content">
                ...
                <div class="modal-footer">
                  <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ console_trans('options.close') }}</button>
                </div>
              </div>
            </div>
          </div>
