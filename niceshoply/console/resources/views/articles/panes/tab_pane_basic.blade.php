<div class="tab-pane fade show active mt-3" id="basic-tab-pane" role="tabpanel" aria-labelledby="basic-tab" tabindex="0">
  ...
  <div class="mb-3 col-12 col-md-8">
    <div class="mb-1 fs-6">{{ __('console/article.title') }}</div>
    @if(has_translator())
      <div
    ...
    @foreach (locales() as $locale)
      ...
      <div class="input-group mb-2">
        ...
        <input type="text" class="form-control" name="translations[{{ $localeCode }}][title]"
                ...
                @if(is_setting_locale($localeCode)) required @endif placeholder="{{ __('console/article.title') }}" aria-label="{{ $localeName }}"
    ...
    <div class="mt-1 text-muted small">
      <i class="bi bi-info-circle me-1"></i>{{ __('console/article.title_required_hint') }}
    </div>
  </div>

  <div class="mb-3 col-12 col-md-8">
    <x-common-form-image title="{{ __('console/article.main_image') }}" name="image"
                        value="{{ old('image', $article->image ?? '') }}"/>
    <div class="mt-2 text-muted small">
      <i class="bi bi-info-circle me-1"></i>{{ __('console/article.main_image_description') }}
    </div>
  </div>
...
@hookinsert('console.article.edit.basic.bottom')