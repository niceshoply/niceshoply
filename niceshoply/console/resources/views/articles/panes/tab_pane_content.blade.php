  ...
  <div class="tab-content" id="locales-content-tabContent">
    @foreach (locales() as $locale)
      <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
        ...
        <div class="mb-3">
          <label class="form-label">{{ __('console/common.summary') }}</label>
          <textarea rows="4" name="translations[{{ $locale->code }}][summary]" class="form-control"
            placeholder="{{ __('console/common.summary') }}">{{ old('translations.' . $locale->code . '.summary', $article->translate($locale->code, 'summary')) }}</textarea>
          <div class="form-text">{{ __('console/common.summary_description') }}</div>
        </div>

        <div class="mb-3">
          <label class="form-label">{{ __('console/article.content') }}</label>
          <textarea rows="16" type="text" name="translations[{{ $locale->code }}][content]"
            ...
            placeholder="{{ __('console/article.content') }}">{{ old('translations.' . $locale->code . '.content', $article->translate($locale->code, 'content')) }}</textarea>
        </div>

        <div class="mb-3">
          <label class="form-label">{{ __('console/article.locale_image') }}</label>
          ...
          <div class="form-text">{{ __('console/article.locale_image_description') }}</div>
        </div>
...
@hookinsert('console.article.edit.content.bottom')