...
@section('content')
  <form class="needs-validation" novalidate id="app-form"
        action="{{ $attribute->id ? console_route('attributes.update', [$attribute->id]) : console_route('attributes.store') }}"
        method="POST">
    @csrf
      ...
      <div class="col-12 col-md-6">
        <div class="card h-min-600">
          <div class="card-header">
            <h5 class="card-title mb-0">{{ __('console/menu.attributes') }}</h5>
          </div>
          <div class="card-body">
            <x-common-form-input title="{{ __('console/common.name') }}" :multiple="true" name="name"
                                 :value="old('translations', $attribute->translations)"
                                 placeholder="{{ __('console/common.name') }}"/>

            <x-common-form-select title="{{ __('console/menu.attribute_groups') }}" name="attribute_group_id"
                                  :options="$attribute_groups" key="id" label="name"
                                  value="{{ old('attribute_group_id', $attribute->attribute_group_id) }}" required
                                  placeholder="{{ __('console/menu.attribute_groups') }}"/>

            <x-common-form-input title="{{ __('console/common.position') }}" name="position"
                                 :value="old('position', $attribute->position)" required
                                 placeholder="{{ __('console/common.position') }}"/>
          </div>
        </div>
      </div>
        ...
        <div class="col-12 col-md-6">
          <div class="card h-min-600">
            <div class="card-header d-flex justify-content-between">
              <h5 class="card-title mb-0">{{ __('console/attribute.attribute_value') }}</h5>
              <button type="button"
                      class="btn btn-sm btn-outline-primary add-value">{{ __('console/common.add') }}</button>
            </div>
            <div class="card-body">
              <table class="table align-middle">
                ...
                <tr>
                  ...
                  <td class="text-end">{{ __('console/common.actions') }}</td>
                </tr>
                </thead>
                <tbody>
                @foreach($attribute->values as $item)
                  <tr data-id="{{ $item->id }}">
                    ...
                    <td class="text-end">
                      <button type="button"
                              class="btn btn-sm btn-outline-primary edit-value">{{ __('console/common.edit') }}</button>
                      <button type="button"
                              class="btn btn-sm btn-outline-danger delete-value">{{ __('console/common.delete') }}</button>
                    </td>
                  </tr>
                @endforeach
                </tbody>
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="addressModalLabel">{{ __('console/attribute.attribute_value') }}</h5>
        ...
        <div class="modal-body">
          <form class="values-input-wrap" action="">
            ...
            <div class="mt-4 d-flex justify-content-center">
              <a type="button" class="btn btn-primary w-50 form-submit">{{ __('console/common.btn_save') }}</a>
            </div>
          </form>
        </div>
      </div>
...
@push('footer')
  <script>
    ...
    $(document).on('click', '.delete-value', function () {
      ...
      $.ajax({
        url: urls.console_base + '/attribute_values/' + id,
        method: 'DELETE',
        dataType: 'json',
        success: (res) => {
    ...
    inno.validateAndSubmitForm('.values-input-wrap', (response) => {
      let url = id ? `${urls.console_base}/attribute_values/${id}` : `${urls.console_base}/attribute_values`;
