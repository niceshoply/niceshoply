...
<div class="card h-min-600">
  <div class="card-header">
    <h5 class="card-title mb-0">{{ __('console/menu.regions') }}</h5>
  </div>
  <div class="card-body">
    <form class="needs-validation mt-3" id="app-form" novalidate action="{{ $region->id ? console_route('regions.update', [$region->id]) : console_route('regions.store') }}" method="POST">
      @csrf
      @method($region->id ? 'PUT' : 'POST')

      <div class="wp-500">
        <x-common-form-input title="{{ __('console/region.name') }}" name="name" :value="old('name', $region->name ?? '')" required placeholder="{{ __('console/region.name') }}" />
        <x-common-form-input title="{{ __('console/region.description') }}" name="description" :value="old('description', $region->description ?? '')" required placeholder="{{ __('console/region.name') }}" />
        <x-common-form-input title="{{ __('console/region.position') }}" name="position" :value="old('position', $region->position ?? 0)" required placeholder="{{ __('console/region.name') }}" />
        <x-console::form.row title="{{ __('console/region.region_states') }}" required>
            ...
            <tbody>
              @foreach($region->regionStates as $index => $item)
                <tr>
                  ...
                  <td class="text-end">
                    <button type="button" class="btn btn-sm btn-outline-danger remove-tax">{{ __('console/common.delete')}}</button>
                  </td>
                </tr>
                @php ($index++)
              @endforeach
            ...
            <tfoot>
              <tr>
                <td colspan="3" class="text-end">
                  <button type="button" class="btn add-tax btn-sm btn-outline-primary">{{ __('console/common.add') }}</button>
                </td>
              </tr>
            </tfoot>
          </table>
