...
@section('title', __('console/menu.sns'))

@section('content')
  <div class="card h-min-600" id="app">
    <div class="card-header  d-flex justify-content-between align-items-center">
      <h5 class="card-title mb-0">{{__('console/sns.pl_config')}}</h5>
      <button @click.prevent="verification" type="submit" class="btn btn-primary">{{__('console/sns.save')}}</button>
    </div>
    <div class="card-body">
      <form class="needs-validation" novalidate id="app-form" action="{{ console_route('sns.index') }}" method="POST">
        @csrf
        ...
        <div class="container mt-1">
          <table class="table table-bordered">
            ...
            <tr class="bg-dark-subtle">
              <th scope="col">{{__('console/sns.type')}}</th>
              <th scope="col">{{__('console/sns.status')}}</th>
              <th scope='col'>{{__('console/sns.client_id')}}</th>
              <th scope="col">{{__('console/sns.client_secret')}}</th>
              <th scope="col">{{__('console/sns.callback')}}</th>
              <th scope="col">{{__('console/common.position')}}</th>
              <th scope="col">{{__('console/common.actions')}}</th>
            </tr>
            </thead>
            <tbody>
            <tr v-for="(item, index) in items" :key="index">
              ...
              <td class="align-middle text-center">
                <el-button size="small" type="danger" plain @click="open(index)">{{__('console/common.delete')}}</el-button>
              </td>
            </tr>
            ...
            <tr v-if="items.length === 0">
              <td colspan="7" style="text-align: center;">
                {{__('console/sns.no_data')}}
                <el-button size="small" type="primary" @click.prevent="add">{{__('console/common.add')}}</el-button>
              </td>
            </tr>
            </tbody>
          </table>
...
@push('footer')
  <script>
    ...
    let app = createApp({
      setup() {
        const successfully = '{{ __('console/sns.copy_successfully') }}';
        const fields = '{{ __('console/sns.required_fields') }}';
        const failure = '{{ __('console/sns.failure') }}';
        const success = '{{ __('console/sns.success') }}';
        ...
        const verification = async () => {
          ...
          try {
            ...
            await axios.post('{{ console_route('socials.store') }}', formData);
            inno.msg(success);
          } catch (error) {
            inno.msg(failure);
            console.error(error);
