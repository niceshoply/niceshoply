@push('header')
    {{-- 文件管理器已迁移至 Vue 3 + Element Plus，复用布局已加载的 Vue 3 / Element Plus，
         不再单独引入 Vue 2.7 / Element UI / unpkg CDN（消除同页双 Vue 主版本冲突）。 --}}
...
@endpush
  ...
  <div id="app">
          ...
          <el-col :span="12">
            <el-button-group>
              <el-button type="primary" size="small" @click="uploadFile">
                <el-icon><Upload></Upload></el-icon> {{ __('console/file_manager.upload') }}
              </el-button>
              <el-button size="small" @click="createFolder">
                <el-icon><FolderAdd></FolderAdd></el-icon> {{ __('console/file_manager.create_folder') }}
              </el-button>
              <el-button size="small" :type="isMultiSelectMode ? 'primary' : 'default'" @click="toggleMultiSelectMode">
                <el-icon><Check></Check></el-icon> {{ __('console/file_manager.multi_select_mode') }}
              </el-button>
              <el-button v-if="isMultiSelectMode" size="small" @click="selectAll">
                <el-icon><Finished></Finished></el-icon> {{ __('console/file_manager.select_all') }}
              </el-button>

              <el-button v-if="isIframeMode" type="primary" size="small" @click="handleConfirm">{{ __('console/file_manager.select_submit') }}</el-button>
          ...
          <el-col :span="12" style="text-align: right">
            <el-button-group>
              <el-button size="small" :disabled="selectedFiles.length !== 1" @click="renameSelectedFile">
                <el-icon><Edit></Edit></el-icon> {{ __('console/file_manager.rename') }}
              </el-button>
              <el-button size="small" :disabled="!selectedFiles.length" @click="deleteFiles">
                <el-icon><Delete></Delete></el-icon> {{ __('console/file_manager.delete') }}
              </el-button>
              <el-button size="small" :disabled="!selectedFiles.length" @click="moveFiles">
                <el-icon><Folder></Folder></el-icon> {{ __('console/file_manager.move_to') }}
              </el-button>
              <el-button size="small" :disabled="!selectedFiles.length" @click="copyFiles">
                <el-icon><DocumentCopy></DocumentCopy></el-icon> {{ __('console/file_manager.copy_to') }}
              </el-button>
            </el-button-group>
            <!-- 排序选择器 -->
            <el-select v-model="sortField" size="small" style="width:110px;margin-left:10px;" @change="onSortChange">
              <el-option label="{{ __('console/file_manager.sort_by_time') }}" value="created"></el-option>
              <el-option label="{{ __('console/file_manager.sort_by_name') }}" value="name"></el-option>
            </el-select>
            <el-select v-model="sortOrder" size="small" style="width:90px;margin-left:5px;" @change="onSortChange">
              <el-option label="{{ __('console/file_manager.sort_desc') }}" value="desc"></el-option>
              <el-option label="{{ __('console/file_manager.sort_asc') }}" value="asc"></el-option>
            </el-select>
          </el-col>
        </el-row>
      </div>
      <el-row :gutter="0">
        ...
        <el-col :xs="8" :sm="6" :md="5" :lg="4"
          ...
          <div class="folder-tree">
            <el-tree ref="folderTree" :key="treeKey" :props="treeProps" :load="loadNode" lazy
              @node-click="handleNodeClick" :highlight-current="true" :default-expanded-keys="defaultExpandedKeys"
              node-key="id" draggable :allow-drop="handleAllowDrop" :allow-drag="handleAllowDrag"
              @node-drag-start="handleDragStart" @node-drag-enter="handleDragEnter" @node-drag-leave="handleDragLeave"
              @node-drag-end="handleNodeDragEnd" @node-drop="handleNodeDrop" class="folder-tree-container">
              <template #default="{ node, data }">
                <span class="el-tree-node__wrapper"
                  ...
                  <span class="custom-tree-node">
                    <el-icon style="margin-right: 4px; color: #d50000;">
                      <FolderOpened v-if="node.expanded"></FolderOpened>
        ...
        <el-col :xs="16" :sm="18" :md="19" :lg="20"
          ...
          <div class="file-list">
            <div class="file-list-content">
              <div v-loading="loading" element-loading-text="{{ __('console/file_manager.loading') }}">
                <template v-if="files.length">
                          ...
                          <template v-else>
                            <div class="file-thumb">
                              <div v-if="file.mime && file.mime.startsWith('image/')" class="preview-button"
                                ...
                                {{ __('console/file_manager.preview') }}
                              </div>
                              <el-image :src="file.url" :alt="file.name" fit="contain"></el-image>
                            </div>
                          </template>
                        </div>
                        <div class="file-info">
                          <p class="file-name" :title="file.name">@{{ file.name }}</p>
                          <p class="file-type">@{{ file.is_dir ? folderTypeLabel : file.mime }}</p>
                        </div>
                      </div>
                    </el-col>
                  </el-row>
                  ...
                  <div class="pagination-container">
                    <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
                      ...
                      prev-text="{{ __('console/file_manager.prev') }}" next-text="{{ __('console/file_manager.next') }}"
                      layout="total, sizes, prev, pager, next, jumper" :total="pagination.total">
                    </el-pagination>
                  </div>
                </template>
                <!-- 空状态 -->
                <el-empty v-if="!files.length" :description="emptyDescription" :image-size="120"></el-empty>
              </div>

              <!-- 图片预览查看器（替代 Element UI 的 el-image preview-src-list 内部 clickHandler） -->
              <el-image-viewer v-if="viewer.visible" :url-list="viewer.urlList" teleported
                @close="viewer.visible = false"></el-image-viewer>
            </div>
          </div>
        </el-col>
      </el-row>
    ...
    <!-- 新建文件夹对话框 -->
    <el-dialog title="新建文件夹" v-model="folderDialog.visible" width="400px">
    ...
    <!-- 上传文件对话框 -->
    <el-dialog title="上传文件" v-model="uploadDialog.visible" width="500px" @open="onUploadDialogOpen">
      <el-upload class="file-uploader" drag multiple :action="uploadUrl" :headers="uploadHeaders"
        ...
        <template #tip>
          <div class="el-upload__tip">支持 jpg、jpeg、png、gif、webp、mp4 格式的文件</div>
          <div class="el-upload__tip">服务器最大上传文件大小: {{ $uploadMaxFileSize ?? '未知' }}</div>
          <div class="el-upload__tip">POST数据最大大小: {{ $postMaxSize ?? '未知' }}</div>
        </template>
      </el-upload>
    </el-dialog>

    <!-- 修改重命名对话框 -->
    <el-dialog title="重命名" v-model="renameDialog.visible" class="rename-dialog" width="500px">
      <el-form :model="renameDialog.form" label-width="100px">
        <el-form-item label="文件名称">
          <el-input v-model="renameDialog.form.newName" placeholder="请输入新名称">
            <template #append><span>.<span v-text="renameDialog.form.extension"></span></span></template>
          </el-input>
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <!-- 移动文件对话框 -->
    <el-dialog title="移动到" v-model="moveDialog.visible" width="400px">
      <el-tree :key="treeKey" :props="treeProps" :load="loadNode" lazy @node-click="handleMoveTargetSelect"
        :highlight-current="true" node-key="id">
        <template #default="{ node }">
          <span class="custom-tree-node">
            <el-icon style="margin-right: 4px; color: #d50000;"><Folder></Folder></el-icon>            <span>@{{ node.label }}</span>
          </span>
        </template>
      </el-tree>
      <template #footer>
    ...
    <div class="file-card-context-menu" v-show="contextMenu.visible" :style="contextMenu.style">
      <ul>
        <li @click="renameFile"><el-icon><Edit></Edit></el-icon> 重命名</li>
        <li @click="deleteFile"><el-icon><Delete></Delete></el-icon> 删除</li>
        <li @click="moveFile"><el-icon><Folder></Folder></el-icon> 移动到</li>
        <li @click="copyFile"><el-icon><DocumentCopy></DocumentCopy></el-icon> 复制到</li>
      </ul>
    </div>

    <!-- 复制文件对话框 -->
    <el-dialog title="复制到" v-model="copyDialog.visible" width="400px">
      <el-tree :key="treeKey" :props="treeProps" :load="loadNode" lazy @node-click="handleCopyTargetSelect"
        :highlight-current="true" node-key="id">
        <template #default="{ node }">
          <span class="custom-tree-node">
            <el-icon style="margin-right: 4px; color: #d50000;"><Folder></Folder></el-icon>            <span>@{{ node.label }}</span>
    ...
    <div v-if="folderContextMenu.visible" class="file-card-context-menu"
      ...
      <ul>
        <li @click="renameFolder">
          <el-icon><Edit></Edit></el-icon> 重命名
        </li>
        <li @click="moveFolder">
          <el-icon><Position></Position></el-icon> 移动到
        </li>
    ...
    <!-- 文件夹重命名对话框 -->
    <el-dialog title="重命名文件夹" v-model="folderRenameDialog.visible" width="400px">
    ...
    <!-- 文件夹移动对话框 -->
    <el-dialog title="移动文件夹" v-model="folderMoveDialog.visible" width="400px">
      <el-tree :key="treeKey" :props="treeProps" :load="loadNode" lazy @node-click="handleFolderMoveTargetSelect"
        :highlight-current="true" node-key="id">
        <template #default="{ node }">
          <span class="custom-tree-node">
            <el-icon style="margin-right: 4px; color: #d50000;"><Folder></Folder></el-icon>            <span>@{{ node.label }}</span>
...
@push('footer')
  <script>
    const fileManagerAppConfig = {
      created() {},
      mounted() {
        // 初始化选中根目录并加载其文件；目录树通过 lazy load 按需展开子级
        this.currentFolder = { id: '/', name: this.rootName, path: '/' };
      ...
      data() {
        return {
          ...
          selectedFiles: [],
          currentFolder: null,
          // 懒加载目录树配置（Element Plus el-tree lazy 模式）
          treeProps: {
            label: 'name',
            children: 'children',
            isLeaf: 'isLeaf'
          },
          // 用于在目录结构变化后强制重挂载目录树（重新触发 lazy 加载）
          treeKey: 0,
          rootName: "{{ __('console/file_manager.root_name') }}",
          folderTypeLabel: @json(__('console/file_manager.folder')),
          emptyDescription: @json(__('console/file_manager.empty')),
          // 图片预览查看器
          viewer: {
            visible: false,
            urlList: []
          ...
          uploadUrl: '/api/v1/console/file_manager/upload',
          uploadHeaders: {
            'Authorization': 'Bearer ' + (document.querySelector('meta[name="api-token"]')?.getAttribute('content') || window.getApiToken?.() || '')
          },
          uploadData: {
            path: '/',
            type: 'images' // 默认上传路径
      ...
      methods: {
        ...
        submitCreateFolder() {
          if (!this.folderDialog.form.name) {
            this.$message.warning("{{ __('console/file_manager.prompt_enter_folder_name') }}");
            return;
          }
          ...
          http.post('file_manager/directories', {
            ...
            if (res.success) {
              this.$message.success("{{ __('console/file_manager.create_success') }}");
              this.folderDialog.visible = false;
              this.folderDialog.form.name = '';
              // 重新加载文件夹树
              this.reloadTree();
            } else {
              this.$message.error(res.message || "{{ __('console/file_manager.create_fail') }}");
            }
          }).catch(err => {
            this.$message.error("{{ __('console/file_manager.create_fail_prefix') }}" + err.message);
          });
        },
        deleteFiles() {
          if (!this.selectedFiles.length) return;

          this.$confirm("{{ __('console/file_manager.confirm_delete_selected_files') }}", "{{ __('console/file_manager.prompt') }}", {
            confirmButtonText: "{{ __('console/file_manager.ok') }}",
            cancelButtonText: "{{ __('console/file_manager.cancel') }}",
            type: 'warning'
          }).then(() => {
        ...
        // 懒加载目录树节点（IMP-14：按层级请求直接子目录，避免整树递归）
        loadNode(node, resolve) {
          ...
          http.get('file_manager/directories', {
            params: { lazy: 1, base_folder: basePath }
          }).then(res => {
            const list = (Array.isArray(res.data) ? res.data : []).map(dir => ({
              id: dir.path,
              name: dir.name,
              path: dir.path,
              isRoot: false,
              // has_children=false 时标记为叶子节点，隐藏展开箭头
              isLeaf: !dir.has_children
            }));
            resolve(list);
          }).catch(err => {
            this.$message.error("{{ __('console/file_manager.error_load_folders_prefix') }}" + err.message);
            resolve([]);
          });
        },

        // 目录结构变化后刷新目录树：通过改变 treeKey 强制重挂载，重新触发 lazy 加载
        reloadTree() {
          this.treeKey++;
          this.currentFolder = { id: '/', name: this.rootName, path: '/' };
          this.defaultExpandedKeys = ['/'];
        ...
        // 打开图片预览查看器
        previewImage(file) {
          this.viewer.urlList = [file.origin_url || file.url];
        ...
        submitRename() {
          if (!this.renameDialog.form.newName) {
            this.$message.warning("{{ __('console/file_manager.enter_new_name') }}");
            return;
          }
          ...
          http.post('file_manager/rename', {
            ...
            if (res.success) {
              this.$message.success("{{ __('console/file_manager.rename_success') }}");
        ...
        deleteFile() {
          const file = this.contextMenu.file;
          this.$confirm("{{ __('console/file_manager.delete_file_confirm') }}", "{{ __('console/file_manager.prompt') }}", {
            confirmButtonText: "{{ __('console/file_manager.ok') }}",
            cancelButtonText: "{{ __('console/file_manager.cancel') }}",
            type: 'warning'
          }).then(() => {
            ...
            http.delete('file_manager/files', {
              ...
              if (res.success) {
                this.$message.success("{{ __('console/file_manager.delete_success') }}");
                this.selectedFiles = [];
                this.loadFiles(currentPath);
              }
            });
        ...
        submitMove() {
          if (!this.moveDialog.targetPath) {
            this.$message.warning("{{ __('console/file_manager.select_target_folder') }}");
            return;
          }
          ...
          http.post('file_manager/move_files', {
            ...
            if (res.success) {
              this.$message.success("{{ __('console/file_manager.move_success') }}");
        ...
        submitCopy() {
          ...
          http.post('file_manager/copy_files', {
            ...
            if (res.success) {
              this.$message.success("{{ __('console/file_manager.copy_success') }}");
        ...
        deleteFolder() {
          const folder = this.folderContextMenu.folder;
          this.$confirm("{{ __('console/file_manager.delete_folder_confirm') }}", "{{ __('console/file_manager.prompt') }}", {
            confirmButtonText: "{{ __('console/file_manager.ok') }}",
            cancelButtonText: "{{ __('console/file_manager.cancel') }}",
            type: 'warning'
          }).then(() => {
        ...
        handleFolderMoveTargetSelect(data) {
          ...
          if (data.path === this.folderMoveDialog.folder.path ||
            ...
            this.$message.warning("{{ __('console/file_manager.cannot_move_to_self') }}");
            return;
          }
          ...
          http.post('file_manager/move_directories', {
            ...
            if (res.success) {
              ...
              if (this.currentFolder && this.currentFolder.path === sourcePath) {
                ...
                this.reloadTree();
            this.$message.error(err.message || "{{ __('console/file_manager.move_fail') }}");
          });
        },
    ...
    // Vue 3 + Element Plus 启动：注册 Element Plus、全部图标组件，
    // 并将 $message / $confirm 等挂到全局属性，保持 Options API 中既有调用不变。
    (function () {
      const app = Vue.createApp(fileManagerAppConfig);
      app.use(ElementPlus);

      // 注册 Element Plus 全部 SVG 图标为全局组件（<el-icon><Folder/></el-icon>）
      ...
      // 兼容原 Vue 2 中的 this.$message / this.$confirm 等调用
      ...
      window.fileManagerApp = app.mount('#app');
    })();
  </script>
@endpush
