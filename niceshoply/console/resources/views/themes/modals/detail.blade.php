        ...
        <ul class="nav nav-tabs mb-4" id="themeDetailTabs{{ $theme['code'] }}" role="tablist">
          ...
          <li class="nav-item" role="presentation">
            <button class="nav-link" 
              ...
              {{ __('console/themes.import_export_data') }}
            </button>
          </li>
        </ul>
            ...
            @if(isset($theme['description']) && $theme['description'])
              <div class="mb-4">
                <div class="text-secondary small text-uppercase mb-2 fw-semibold">
                  ...
                  {{ __('console/themes.theme_description') }}
                </div>
                  ...
                  <div>
                    @if($theme['selected'])
                      <span class="badge bg-success">
                        <i class="bi bi-check-circle-fill me-1"></i>
                        {{ __('console/common.active') }}
                      </span>
                    @else
                      <span class="badge bg-secondary">
                        <i class="bi bi-circle me-1"></i>
                        {{ __('console/common.not_used') }}
                      </span>
                    @endif
                  </div>
                </div>
            ...
            @if($theme['has_demo'])
              ...
              <div class="alert alert-warning border mb-4">
                <div class="d-flex gap-3">
                  ...
                  <div class="flex-grow-1">
                    <div class="fw-semibold mb-2">{{ __('console/themes.demo_data_notice') }}</div>
                    <div class="text-secondary small">
                      {{ __('console/themes.demo_data_warning') }}
                    </div>
                  </div>
                </div>
              </div>
            ...
            @else
              ...
              <div class="alert alert-info border mb-4">
                <div class="d-flex gap-3">
                  ...
                  <div class="flex-grow-1">
                    <div class="fw-semibold mb-2">{{ __('console/themes.no_demo_data') }}</div>
                    <div class="text-secondary small">
                      {{ __('console/themes.no_demo_data_description', ['code' => $theme['code']]) }}
                    </div>
                  </div>
                </div>
              </div>
...
@if($theme['has_demo'])
  @include('console::themes.modals.import-confirm', ['theme' => $theme])
@endif
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (exportSqlBtn) {
    exportSqlBtn.addEventListener('click', function() {
      ...
      let url = '{{ console_route('themes.export_sql', $theme['code']) }}';
      ...
      setTimeout(() => {
        layer.closeAll('loading');
        inno.msg('{{ __('console/themes.export_started') }}');
      }, 500);
    });
  }
