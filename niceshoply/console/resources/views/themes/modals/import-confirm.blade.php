  ...
  <div class="modal-dialog modal-sm modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">{{ __('console/themes.confirm_import') }}</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body">
        <p class="mb-0">{{ __('console/themes.confirm_import_warning') }}</p>
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">{{ __('console/common.cancel') }}</button>
        <button type="button" class="btn btn-primary" id="confirmImportDemo{{ $theme['code'] }}">
          ...
          {{ __('console/themes.confirm_import_button') }}
        </button>
      </div>
    </div>
  </div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (confirmBtn) {
    confirmBtn.addEventListener('click', function() {
      ...
      // Send import request
      fetch('{{ console_route('themes.import_demo', $theme['code']) }}', {
        method: 'POST',
        headers: {
        ...
        return response.json().then(data => {
          if (response.ok && data.success) {
            ...
            inno.msg(data.message || '{{ __('console/themes.demo_installed') }}');
            ...
            // Show error message
            const errorText = data.message || data.error || '{{ __('console/themes.import_failed') }}';
        ...
        // Show error message
        const errorText = error.message || '{{ __('console/themes.import_failed') }}';
        errorMsg.textContent = errorText;
        errorDiv.classList.remove('d-none');
        // Scroll to error
        errorDiv.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
