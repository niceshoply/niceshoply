...
@section('title', __('console/menu.themes_settings'))
...
@section('content')
  <form class="needs-validation" novalidate action="{{ console_route('themes_settings.update') }}" method="POST"
        id="app-form">
    @csrf
    @method('put')
    <div class="row">
      <div class="col-3">
        <div class="card h-min-600" id="setting-menu">
          <div class="card-header">{{ __('console/menu.themes_settings') }}</div>
          <div class="card-body">
            <ul class="nav flex-column settings-nav">
              <a class="nav-link active" href="#" data-bs-toggle="tab"
                 data-bs-target="#tab-setting-header-menu">{{ __('console/setting.header_menu') }}</a>
              <a class="nav-link" href="#" data-bs-toggle="tab"
                 data-bs-target="#tab-setting-footer-menu">{{ __('console/setting.footer_menu') }}</a>
              <a class="nav-link" href="#" data-bs-toggle="tab"
                 data-bs-target="#tab-setting-slideshow">{{ __('console/setting.slideshow') }}</a>
              <a class="nav-link" href="#" data-bs-toggle="tab"
                 data-bs-target="#tab-setting-hot-products">{{ __('console/setting.hot_products') }}</a>
              <a class="nav-link" href="#" data-bs-toggle="tab"
                 data-bs-target="#tab-setting-home-categories">{{ __('console/setting.home_categories') }}</a>
              @hookinsert('console.themes.settings.hot_products.tab')
            </ul>
          </div>
        </div>
      </div>
      <div class="col-9">
        <div class="card h-min-600">
          <div class="card-header setting-header">{{ __('console/setting.header_menu') }}</div>
          <div class="card-body">
            <div class="tab-content">
              {{-- 顶部菜单 / 底部菜单 / 轮播图：已拆分为独立 partial，便于复用与维护 --}}
              @include('console::themes.partials.header_menu')

              @include('console::themes.partials.footer_menu')

              @include('console::themes.partials.slideshow')

              <div class="tab-pane fade" id="tab-setting-hot-products">
                <!-- 推荐商品设置 -->
                <div class="mb-4">
                  <h5 class="mb-3">{{ __('console/setting.hot_products') }}</h5>
                  
                  <!-- 按分类组织商品 -->
                  <div class="mb-3">
                    <button type="button" class="btn btn-success" id="addCategoryGroupBtn">
                      <i class="bi bi-plus-circle"></i> {{ __('console/setting.add_category') }}
                    </button>
                  </div>
              ...
              <div class="tab-pane fade" id="tab-setting-home-categories">
                <!-- 首页分类设置 -->
                <div class="mb-4">
                  <h5 class="mb-3">{{ __('console/setting.home_categories') }}</h5>
                  <p class="text-muted small mb-3">{{ __('console/setting.home_categories_desc') }}</p>
                  ...
                  <div class="row">
                    ...
                    <div class="col-6">
                      <div class="card">
                        <div class="card-header">{{ __('console/setting.selected_categories') }}</div>
                        <div class="card-body hp-400 overflow-y-auto">
                          <div id="selectedHomeCategories" class="list-group">
                            <!-- Selected categories will be displayed here -->
                          </div>
                          <p class="text-muted small mt-3">{{ __('console/setting.home_categories_hint') }}</p>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              @hookinsert('console.themes.settings.hot_products.tab_content')
            </div>
          </div>
        </div>
      </div>
...
@push('footer')
  <script>
    ...
    function renderProductsInCategory(groupIndex, productIds) {
      ...
      productIds.forEach((productId) => {
        ...
        const imageUrl = productImage || '{{ asset("vendor/NiceShoply/images/placeholder.png") }}';
    ...
    $(document).on('click', '.add-product-to-category', function() {
      ...
      layer.open({
        ...
        shade: 0.8,
        area: ["800px", "800px"],
        content: urls.console_base + '/products/selector',
      });
    });
    ...
    function loadHotProductsInfo() {
      ...
      // 通过API获取商品详细信息
      $.ajax({
        url: urls.console_api + '/products/names',
        type: 'GET',
        data: {
    ...
    function renderSelectedHomeCategories() {
      ...
      if (selectedIds.length === 0) {
        selectedDiv.html('<p class="text-muted small">' + '{{ __('console/setting.no_categories_selected') ?? "未选择分类" }}' + '</p>');
        return;
      }
  ...
  <!-- 供应商相关脚本（通过 Hook 扩展） -->
  @hookinsert('console.themes.settings.script.after')
@endpush