...
@section('title', __('console/menu.attribute_groups'))
...
<div class="card h-min-600" id="app">
  <div class="card-body">
    ...
    <ul class="nav nav-tabs mb-3">
      <li class="nav-item">
        <a class="nav-link" href="{{ console_route('attributes.index') }}">
          <i class="bi bi-tags"></i> {{ __('console/menu.attributes') }}
        </a>
      </li>
      <li class="nav-item">
        <a class="nav-link active" href="{{ console_route('attribute_groups.index') }}">
          <i class="bi bi-collection"></i> {{ __('console/menu.attribute_groups') }}
        </a>
      </li>
    </ul>

    <x-console-data-criteria :criteria="$criteria ?? []" :action="console_route('attribute_groups.index')" />
    ...
    <div class="table-responsive">
      ...
      __('console/common.edit')}}</el-button>
          ...
          <div>
          <form ref="deleteForm" action="{{ console_route('attribute_groups.destroy', [$item->id]) }}"
...
@endif
  ...
  <el-drawer v-model="drawer" size="500" @close="close">
    <template #header>
      <div class="text-dark fs-4">{{ __('console/menu.attribute_groups') }}</div>
    </template>
    <el-form ref="formRef" label-position="top" :model="form" :rules="rules" label-width="auto" status-icon>
          ...
          <div class="tab-content pb-3">
            <div :class="['tab-pane fade', !index ? 'show active' : '']" :id="'values-' + locale . code + '-pane'"
              ...
              <el-input size="large" v-model="form.translations[locale.code].name"
                placeholder="{{ __('console/common.name')}}"></el-input>
            </div>
          </div>
        </div>
      </el-form-item>
      ...
      <el-form-item label="{{ __('console/common.position')}}" prop="position">
        <el-input size="large" v-model="form.position" placeholder="{{ __('console/common.position')}}"></el-input>
      </el-form-item>
    </el-form>
...
@push('footer')
    ...
    const api = @json(console_route('attribute_groups.index'));
    ...
    .then(() => {
      const deleteUrl = urls.console_base + '/attribute_groups/' + id;
