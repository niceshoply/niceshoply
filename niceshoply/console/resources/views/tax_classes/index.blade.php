...
@section('title', __('console/menu.tax_classes'))
...
<div class="card h-min-600" id="app">
  <div class="card-body">
    ...
    <x-console-data-criteria :criteria="$criteria ?? []" :action="console_route('tax_classes.index')" />
      ...
      <table class="table align-middle">
        <thead>
          <tr>
            ...
            <td>{{ __('console/common.description') }}</td>
          ...
          <tr>
            ...
            <td>
              <div class="d-flex gap-1">
                ...
                <form ref="deleteForm" action="{{ console_route('tax_classes.destroy', [$item->id]) }}" method="POST"
                  class="d-inline">
                  @csrf
                  @method('DELETE')
    ...
    {{ $tax_classes->withQueryString()->links('console::vendor/pagination/bootstrap-4') }}
    @else
    <x-common-no-data />
    @endif
  </div>
  ...
  <el-drawer v-model="drawer" size="500" @close="close">
    <template #header>
      <div class="text-dark fs-4">{{ __('console/menu.tax_classes') }}</div>
    </template>
    <el-form ref="formRef" label-position="top" :model="form" :rules="rules" label-width="auto" status-icon>
      <el-form-item label="{{ __('console/common.name') }}" prop="name">
        <el-input size="large" v-model="form.name" placeholder="{{ __('console/common.name') }}"></el-input>
      </el-form-item>

      <el-form-item label="{{ __('console/common.description') }}" prop="description">
        <el-input size="large" v-model="form.description" placeholder="{{ __('console/common.description') }}"></el-input>
      </el-form-item>

      <el-form-item label="{{ __('console/tax_classes.rule') }}">
        <table class="table table-bordered regions-table">
          <thead>
            <tr>
              <th width="30%">{{ __('console/tax_classes.tax_rate_id') }}</th>
              <th width="30%">{{ __('console/tax_classes.based') }}</th>
              <th width="20%">{{ __('console/tax_classes.priority') }}</th>
            ...
            <tr v-for="(item, index) in form.tax_rules" :key="index">
              ...
              <td>
                <input type="text" class="form-control form-control-sm" v-model="item.priority"
                  placeholder="{{ __('console/tax_classes.priority') }}">
              </td>
              <td class="text-end">
                <el-button type="danger" @click="form.tax_rules.splice(index, 1)">{{ __('console/common.delete')}}
                </el-button>
              </td>
            </tr>
          </tbody>
          <tfoot>
            <tr>
              <td colspan="4" class="text-end">
                <el-button type="primary" @click="addItem">{{ __('console/common.add') }}</el-button>
              </td>
            </tr>
          </tfoot>
        </table>
...
<script>
  const { createApp, ref, reactive, onMounted, getCurrentInstance } = Vue;
    ...
    const api = @json(console_route('tax_classes.index'));
    ...
    .then(() => {
    const deletUrl = urls.console_base + '/tax_classes/' + itemId;
