...
@section('title', __('console/menu.countries'))
...
<div class="card h-min-600" id="app">
  <div class="card-body">
    ...
    <x-console-data-criteria :criteria="$criteria ?? []" :action="console_route('countries.index')" />
      ...
      <table class="table align-middle">
        <thead>
          <tr>
            ...
            <td>{{ __('console/country.continent') }}</td>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>{{ $item['position'] }}</td>
            <td>
              @include('console::shared.list_switch', ['value' => $item->active, 'url' => console_route('countries.active',
              $item->id)])
            </td>
            <td>
              <div class="d-flex gap-2">
                ...
                <div>
                  <form ref="deleteForm" action="{{ console_route('countries.destroy', [$item->id]) }}" method="POST"
                    class="d-inline">
                    @csrf
                    @method('DELETE')
    ...
    {{ $countries->withQueryString()->links('console::vendor/pagination/bootstrap-4') }}
    @else
    <x-common-no-data />
    @endif
  </div>
  ...
  <el-drawer v-model="drawer" size="500" @close="close">
    <template #header>
      <div class="text-dark fs-4">{{ __('console/menu.countries') }}</div>
    </template>
    <el-form ref="formRef" label-position="top" :model="form" :rules="rules" label-width="auto" status-icon>
      ...
      <el-form-item label="{{ __('console/country.continent') }}" prop="continent">
        <el-input v-model="form.continent"></el-input>
      </el-form-item>

      <el-form-item label="{{ __('console/common.position') }}" prop="position">
        <el-input v-model="form.position"></el-input>
      </el-form-item>

      <el-form-item label="{{ __('console/common.status') }}" prop="active">
...
@push('footer')
<script>
  const api = @json(console_route('countries.index'));
      ...
      setup() {
        ...
        const open = (index) => {
          ElMessageBox.confirm(
            ...
            .then(() => {
              const deleteUrl = urls.console_base + '/countries/' + index;
