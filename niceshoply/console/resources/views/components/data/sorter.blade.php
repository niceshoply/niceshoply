...
<div class="sorter-component d-flex align-items-center gap-2">
  <div class="text-muted sorter-label">{{ __('console/common.sort') }}:</div>
  <select class="form-select form-select-sm sorter-field" id="sort-field" name="sort">
    <option value="">{{ __('console/common.none') }}</option>
  ...
  <select class="form-select form-select-sm sorter-direction" id="sort-direction" name="order">
    <option value="" class="none-option" {{ !request('sort') ? '' : 'style=display:none' }}>{{ __('console/common.none') }}</option>
    <option value="desc" {{ request('order', 'desc') == 'desc' ? 'selected' : '' }}>{{ __('console/common.desc') }} ↓</option>
    <option value="asc" {{ request('order') == 'asc' ? 'selected' : '' }}>{{ __('console/common.asc') }} ↑</option>
  </select>
</div>
