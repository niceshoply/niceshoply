  ...
  <form action="{{ $action }}" method="GET" id="filter-form">
    ...
    <div class="d-flex justify-content-between align-items-center px-3 py-2 filter-header">
      <div class="d-flex align-items-center">
        <i class="bi bi-funnel me-1 text-secondary"></i> <span class="fw-medium">{{ __('console/common.filter') }}</span>
        <a href="#" class="text-decoration-none small text-primary ms-3 toggle-filters">
          <i class="bi bi-chevron-{{ $hasFilters ? 'up' : 'down' }}"></i>
          <span>{{ $hasFilters ? __('console/common.collapse') : __('console/common.expand') }}</span>
        </a>
      </div>
      <div class="d-flex align-items-center">
        ...
        <a href="{{ $action }}" class="btn btn-sm btn-outline-secondary @if($export) me-2 @endif">
          <i class="bi bi-arrow-repeat"></i> {{ __('console/common.reset') }}
        </a>
        @if($export)
          <a href="{{ $getExportUrl() }}" class="btn btn-sm btn-outline-secondary">
            <i class="bi bi-file-earmark-excel"></i> {{ __('console/common.export') }}
          </a>
        @endif
      </div>
    </div>
    ...
    <div id="filter-container" class="px-3 py-2 border-top" style="display: {{ $hasFilters ? 'block' : 'none' }};">
      ...
      <div class="row g-2 mb-0">
        @foreach($criteria as $item)
          @include('console::components.data.criteria.' . $item['type'], ['item' => $item])
        @endforeach
      </div>
    </div>
  </form>
...
$(document).ready(function() {
  ...
  $('.toggle-filters').on('click', function(e) {
    ...
    if ($container.is(':visible')) {
      $container.slideUp(200, function() {
        ...
        $text.text('{{ __('console/common.expand') }}');
      });
    } else {
      $container.addClass('border-top').slideDown(200, function() {
        $icon.removeClass('bi-chevron-down').addClass('bi-chevron-up');
        $text.text('{{ __('console/common.collapse') }}');
      });
    }
  });
  ...
  $('#filter-form').on('submit', function() {
    ...
    $('button[type="submit"]').prop('disabled', true).html('<i class="bi bi-hourglass-split"></i> {{ __('console/common.loading') }}');
  });
});
</script>
@endpush
