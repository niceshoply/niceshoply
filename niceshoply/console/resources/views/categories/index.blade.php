...
@section('page-title-right')
  <a href="{{ console_route('categories.create') }}" class="btn btn-primary"><i
        class="bi bi-plus-square"></i> {{ __('console/common.create') }}</a>
  @hookinsert('console.categories.index.page-title-right')
@endsection
...
@push('footer')
<script>
  const api = @json(console_route('categories.index'));
  const categories = @json($categories);
  const categoryIndexUrl = @json(console_route('categories.index'));
        ...
        <h2 class="accordion-header" id="heading${itemId}">
          <div class="d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex align-items-center tool-btn" data-id="${item.id}">
              ...
              <a href="${categoryIndexUrl}/${item.id}/edit" class="btn btn-sm text-nowrap edit-category btn-outline-primary ms-3">{{ __('console/common.edit')}}</a>
              <span class="btn btn-sm ms-2 text-nowrap btn-outline-danger btn-delete">{{ __('console/common.delete')}}</span>
            </div>
          </div>
        </h2>`;
