  ...
  <div class="tab-content" id="locales-content-tabContent">
    @foreach (locales() as $locale)
      <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
        ...
        {{-- 分类介绍 --}}
        <div class="mb-3">
          <label class="form-label">{{ console_trans('category.summary') }}</label>
          <textarea rows="3" name="translations[{{ $locale->code }}][summary]" class="form-control"
            placeholder="{{ console_trans('category.summary') }}">{{ old('translations.' . $locale->code . '.summary', $category->translate($locale->code, 'summary')) }}</textarea>
          <div class="mt-2 text-muted small">
            <i class="bi bi-info-circle me-1"></i>{{ console_trans('category.summary_description') }}
          </div>
        </div>

        {{-- 分类详细描述 --}}
        <div class="mb-3">
          <label class="form-label">{{ console_trans('category.content') }}</label>
          <x-common-form-rich-text name="translations[{{ $locale->code }}][content]"
                                   ...
                                   placeholder="{{ console_trans('category.content') }}"
                                   data-locale="{{ $locale->code }}"/>
          <div class="mt-2 text-muted small">
            <i class="bi bi-info-circle me-1"></i>{{ console_trans('category.content_description') }}
          </div>
        </div>
