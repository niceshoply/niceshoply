<div class="tab-pane fade show active mt-3" id="basic-tab-pane" role="tabpanel" aria-labelledby="basic-tab" tabindex="0">
  ...
  <div class="mb-3 col-12 col-md-6">
    <div class="mb-1 fs-6">{{ console_trans('category.name') }}</div>
    @if(has_translator())
      <div
    ...
    @foreach (locales() as $locale)
      ...
      <div class="input-group mb-2">
        ...
        <input type="text" class="form-control" name="translations[{{ $localeCode }}][name]"
               ...
               required placeholder="{{ console_trans('category.name') }}" aria-label="{{ $localeName }}"
    ...
    <div class="mt-1 text-muted small">
      <i class="bi bi-info-circle me-1"></i>{{ console_trans('category.name') }}{{ console_trans('category.name_required') }}
    </div>
  </div>

  {{-- 主图片（统一，不区分语言） --}}
  <div class="mb-3 col-12 col-md-6">
    <x-common-form-image title="{{ console_trans('category.image') }}" name="image"
                        value="{{ old('image', $category->image ?? '') }}"/>
    <div class="mt-2 text-muted small">
      <i class="bi bi-info-circle me-1"></i>{{ console_trans('category.image_description') }}
    </div>
  </div>

  {{-- 启用状态 --}}
  <div class="mb-3 col-12 col-md-6">
    <x-common-form-switch-radio title="{{ console_trans('common.whether_enable') }}" name="active"
      :value="old('active', $category->active ?? true)" 
      placeholder="{{ console_trans('common.whether_enable') }}" />
  </div>
</div>

@hookinsert('console.category.edit.basic.bottom')