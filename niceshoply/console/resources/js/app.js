...
const UI = {
  ...
  initTabNavigation: () => {
    // 设置页由 SettingsTabs 独立管理，避免与 Bootstrap Tab 双重切换导致 fade 叠层半透明
  ...
  // 顶栏下拉：悬停与点击均走 Bootstrap Popper，避免 CSS 硬编码位移导致位置不一致
  initHeaderDropdowns: () => {
    if (typeof bootstrap === "undefined") {
      return;
    }

    document.querySelectorAll(".header-box .right-tool .header-item.dropdown").forEach((item) => {
  ...
  initHoverEffects: () => {
    ...
    $(".plugin-market-nav-item")
      .mouseenter(function () {
        $(this).addClass("console-item-hover");
      })
      .mouseleave(function () {
        $(this).removeClass("console-item-hover");
      });
  },
  ...
  initAIGenerate: () => {
    $(document).on("click", ".ai-generate", function (e) {
      ...
      const aiUrl = urls.console_ai || urls.ai_generate;
      const statusUrl = urls.console_ai_status || urls.ai_status;
      ...
      axios
        ...
        .then(function (res) {
          ...
          if (res.data && res.data.task_id) {
            pollAIResult(statusUrl, res.data.task_id, $input);
            return;
          }
          // Sync mode: apply result directly
    ...
    function pollAIResult(statusUrl, taskId, $input, attempts) {
      attempts = attempts || 0;
      if (attempts > 60) {
        layer.closeAll("loading");
        layer.msg('AI generation timed out', { icon: 2 });
        return;
      }
      setTimeout(function () {
        axios.get(statusUrl, { params: { task_id: taskId } })
          .then(function (res) {
            ...
            if (status === 'completed') {
              ...
              layer.closeAll("loading");
              layer.msg(res.data.message || 'AI generation failed', { icon: 2 });
            } else {
              pollAIResult(statusUrl, taskId, $input, attempts + 1);
            }
          })
          .catch(function () {
            pollAIResult(statusUrl, taskId, $input, attempts + 1);
...
const FileManager = {
  init: function(callback, options = {}) {
    ...
    layer.open({
      ...
      shade: 0.8,
      area: ["90%", "90%"],
      content: `${urls.console_base}/file_manager/iframe?type=${finalOptions.type}&multiple=${finalOptions.multiple ? "1" : "0"}`,
    });
  }
};
  ...
  setupEditor: (ed) => {
    ...
    ed.on("paste", (e) => {
      ...
      for (let i = 0; i < clipboardData.items.length; i++) {
        ...
        axios
          .post(`${urls.console_api}/file_manager/upload`, formData)
          .then((response) => {
