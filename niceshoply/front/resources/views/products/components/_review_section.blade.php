...
@if(isset($reviewStats) && ($reviewStats['total'] ?? 0) > 0)
  <div class="mb-3 text-secondary">
    {{ __('front/review.positive_rate', ['rate' => $reviewStats['rate'], 'total' => $reviewStats['total']]) }}
  </div>
@endif

<div class="review-filters mb-3 d-flex flex-wrap gap-2 align-items-center">
  <span class="text-muted">{{ __('front/review.filter') }}:</span>
  <a href="javascript:void(0)" class="btn btn-sm btn-outline-secondary review-filter" data-rating="" data-has-images="0">{{ __('front/review.filter_all') }}</a>
  @for($i = 5; $i >= 1; $i--)
    <a href="javascript:void(0)" class="btn btn-sm btn-outline-secondary review-filter" data-rating="{{ $i }}" data-has-images="0">{{ $i }}★</a>
  @endfor
  <a href="javascript:void(0)" class="btn btn-sm btn-outline-secondary review-filter" data-rating="" data-has-images="1">{{ __('front/review.filter_with_images') }}</a>
</div>
...
@push('footer')
  <script>
    $(document).ready(function () {
      $('.load-more-reviews').on('click', function () {
        ...
        axios.get(`{{ front_route('products.reviews', ['product' => $product->id]) }}`, {
          params: {
            page: page,
            rating: button.data('rating') || undefined,
            has_images: button.data('has-images') || undefined,
          }
    ...
    $('.review-filter').on('click', function () {
      ...
      const hasImages = $(this).data('has-images');
      const loadBtn = $('.load-more-reviews');
      loadBtn.data('page', 1).data('rating', rating).data('has-images', hasImages);

      axios.get(`{{ front_route('products.reviews', ['product' => $product->id]) }}`, {
        params: {
          page: 1,
          rating: rating || undefined,
          has_images: hasImages ? 1 : undefined,
        }
      }).then(function (response) {
        if (response.success) {
          $('.review-list-container').html(response.data.html);
          if (response.data.has_more) {
            if (!loadBtn.length) {
              $('.review-list-container').after('<div class="text-center mt-3"><button class="btn btn-outline-primary load-more-reviews" data-page="2" data-product-id="{{ $product->id }}">{{ __('front/common.load_more') }}</button></div>');
            }
          } else {
            loadBtn.parent().remove();
          }
        }
      });
    });
