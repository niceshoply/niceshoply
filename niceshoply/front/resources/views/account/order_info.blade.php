{{--
  ...
  【文件说明】
    用户中心 — 订单详情页。
    展示订单基本信息、商品列表（含规格/选项/子类型）、费用明细、
    收货地址/账单地址、物流信息、订单备注，以及历史操作记录。
    根据订单状态动态显示操作按钮：
      unpaid    → 继续支付 + 取消订单
      completed → 申请退货/退款（RMA）
      shipped   → 确认收货
  ...
  【对应路由/控制器】
    路由名称（GET）：account.orders.number_show
    URL 示例：/{locale}/account/orders/{number}
    控制器：Front\Account\OrderController@show
    参数：$order->number — 订单编号（字符串）

  【可用变量】
    $order       — 订单模型对象，含以下关联/属性：
                     number            订单编号
                     ...
                     comment           买家留言（可为空）
                     fees              费用附加项集合（如运费、优惠券等）
                     shipments         物流记录集合
                     histories         订单状态变更历史集合
                     billing_*         账单地址相关字段（billing_customer_name 等）
                     shipping_*        收货地址相关字段（shipping_customer_name 等）
    $order_items — array，订单商品列表，每项含：
                     name              商品名称
                     product_sku       SKU 编号
                     image             商品图片 URL
                     variant_label     规格描述（如"颜色: 红色"）
                     item_type         商品类型（'normal' 普通商品）
                     item_type_label   类型标签文本（如"赠品"）
                     options           自定义选项数组（含 option_name/option_value_name/price_adjustment）
                     price_format      单价（格式化）
                     quantity          数量
                     subtotal_format   小计（格式化）
                     id                订单项 ID（用于评价）
                     order_number      所属订单编号

  【Sections】
    body-class → 'page-order-info'
    content    → 订单详情主体 + 评价弹窗 + 物流详情弹窗
    footer     → 查看物流轨迹/确认收货/取消订单/评价弹窗填充 JS（@push）

  【插件钩子】
    @hookinsert('account.order_info.top')    — 容器顶部
    @hookinsert('account.order_info.bottom') — 容器底部（位于弹窗区域后）

  【子视图引用】
    shared.review — 评价表单（在 Bootstrap Modal #addReview-Modal 中渲染）

  【自定义建议】
    - 费用明细行通过 $order->fees 循环渲染，每行含 title 和 value_format
    - 物流轨迹弹窗通过 AJAX 请求 front_api/console/shipments/{id}/traces 获取数据
    - 评价只在订单状态为 completed 且该商品未评价（ReviewRepo::orderReviewed）时显示按钮
    - 可在 fees 循环前添加折扣码显示逻辑
  ============================================================
--}}
{{--
  ============================================================
  【文件说明】
    用户中心 — 订单详情页。
    展示单笔订单的完整信息：订单基本信息（编号、时间、状态、总额）、
    商品明细列表（含自定义选项/规格标签）、各项费用（运费、税等）、
    收货地址与账单地址、订单备注、物流发货记录（可查看运单轨迹）、
    订单状态变更历史。
    已完成订单还可发起退货申请（RMA），待付款订单支持继续付款和取消。
  ...
  【对应路由/控制器】
    路由名称（GET）：account.orders.number_show
    URL 示例：/{locale}/account/orders/{order_number}
    控制器：Front\Account\OrderController@show
  ...
  【可用变量】
    $order        — 订单对象，含：
                      ...
                      created_at         创建时间（Carbon）
                      comment            买家备注
                      fees               附加费用集合（运费/税/折扣等，含 title/value_format）
                      shipments          发货记录集合（含 express_code/company/number）
                      histories          状态变更历史集合（含 status/comment/created_at）
                      shipping_*         收货地址字段（name/telephone/zipcode/address_1/2/city/state/country）
                      billing_*          账单地址字段（同上）
    $order_items  — 商品明细数组，每条含：
                      id / name / image / product_sku / variant_label / item_type / item_type_label
                      price_format / quantity / subtotal_format
                      options            自定义选项数组（option_name/option_value_name/price_adjustment）
                      order_number       所属订单编号

  【Sections】
    body-class → 'page-order-info'
    content    → 订单详情主体 + 评价弹窗 + 物流弹窗
    footer     → 评价/确认收货/取消订单/查看物流轨迹 JS（@push）

  【插件钩子】
    @hookinsert('account.order_info.top')    — 内容顶部
    @hookinsert('account.order_info.bottom') — 内容底部

  【自定义建议】
    - 操作按钮区（unpaid/shipped/completed 状态显示不同按钮）可按业务需求增减
    - 物流轨迹通过 AJAX GET ${urls.front_api}/console/shipments/{id}/traces 获取
    - 评价弹窗通过 shared.review 局部模板渲染，触发时回写 data-* 属性到表单字段
  ============================================================
--}}
@extends('layouts.app')
            ...
            <table class="table products-table align-middle">
              ...
              @foreach ($order_items as $product)
                <tr>
                  ...
                  <td>
                    @php($reviewed = \NiceShoply\Common\Repositories\ReviewRepo::orderReviewed(current_customer_id(), $product['id']))
      ...
      <script>
        $(document).ready(function () {
          ...
          $(document).on('click', '[id^="view-shipment-"]', function () {
            ...
            axios.get(`${urls.front_api}/console/shipments/${shipmentId}/traces`)
              .then(response => {
