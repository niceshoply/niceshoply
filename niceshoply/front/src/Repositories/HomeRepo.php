...
class HomeRepo
{
    ...
    public function formatProductData(Product $product): array
    {
        ...
        $supplier      = null;
        $supplierBadge = null;
        if (class_exists('NiceShoply\Supplier\Models\Supplier') && isset($product->supplier_id) && $product->supplier_id) {
            try {
                $supplierModel = \NiceShoply\Supplier\Models\Supplier::query()->find($product->supplier_id);
                if ($supplierModel) {
