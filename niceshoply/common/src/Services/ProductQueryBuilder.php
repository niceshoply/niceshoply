...
class ProductQueryBuilder
{
    ...
    public function applySearchFilters(Builder $builder, array $filters): Builder
    {
        ...
        // 已接入搜索引擎（Scout/Meilisearch）时，用引擎检索得到的 ID 约束查询，
        // 兼顾相关性与大规模 catalog 性能；未启用时回退到数据库 LIKE。
        if (config('scout.enabled')) {
            $ids = Product::search($keyword)->take(1000)->keys()->all();

            // 用 ?: [0] 确保无命中时返回空结果而非全量。
            return $builder->whereIn('products.id', $ids ?: [0]);
        }

        // Fallback: database LIKE search.
        // 用嵌套 where 包裹 OR 条件，避免 orWhereHas 泄漏到其它顶层条件（如 active）。
        $builder->where(function (Builder $query) use ($keyword) {
            $query->whereHas('translation', function (Builder $sub) use ($keyword) {
                $sub->where('name', 'like', "%$keyword%");
            })->orWhereHas('skus', function (Builder $sub) use ($keyword) {
