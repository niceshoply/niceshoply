 ...
 * 访问与转化统计聚合服务
 *
 * 提供每日聚合（visit_daily / conversion_daily）与按周期查询统计。
 */
class VisitStatisticsService
{
    /**
     * 聚合指定日期的每日统计（通常由计划任务在每日凌晨调用）。
     *
    ...
    {
        return $this->getRangeStatistics($date->copy()->startOfWeek(), $date->copy()->endOfWeek());
    }
    ...
    {
        return $this->getRangeStatistics($date->copy()->startOfMonth(), $date->copy()->endOfMonth());
    }
    ...
    {
        return $this->getRangeStatistics($date->copy()->startOfYear(), $date->copy()->endOfYear());
    }
