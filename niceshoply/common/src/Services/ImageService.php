...
class ImageService
{
    ...
    public function __construct($image)
    {
        ...
        // Detect remote (cloud-stored) images
        if (Str::startsWith($image, 'http')) {
            $this->isRemote  = true;
            $this->remoteUrl = $image;
            // Try to download to temp for processing
            $tempPath = $this->downloadToTemp($image);
            if ($tempPath) {
                $this->image     = $image;
    ...
    /**
     * Download a remote image to a temporary local file for processing.
     *
     * @param  string  $url
    ...
    private function downloadToTemp(string $url): ?string
    {
        try {
            ...
            $tempPath = storage_path("app/temp/remote-images/{$hash}.{$ext}");

            // Use cached version if fresh enough (1 hour)
            if (is_file($tempPath) && (time() - filemtime($tempPath)) < 3600) {
                return $tempPath;
            }

            create_directories(dirname($tempPath));
            ...
            Log::warning('Failed to download remote image', [
                'url'   => $url,
                'error' => $e->getMessage(),
            ]);
