...
use NiceShoply\Common\Repositories\CartItemRepo;
use NiceShoply\Common\Resources\CartListItem;
use Throwable;
...
class CartService
{
    ...
    public function addCart($data): array
    {
        ...
        // 验证最低起订量
        $minimumSku = \NiceShoply\Common\Models\Product\Sku::find($data['sku_id']);
        if ($minimumSku && $minimumSku->product) {
            $minimum  = (int) ($minimumSku->product->minimum ?? 1);
        ...
        if (! empty($data['options'])) {
            $productSku = \NiceShoply\Common\Models\Product\Sku::find($data['sku_id']);
            if ($productSku && $productSku->product) {
                $optionService = \NiceShoply\Common\Services\ProductOptionService::getInstance($productSku->product);
    ...
    public function updateCart($cartItem, $data): array
    {
        ...
        // 验证最低起订量
        if ($cartItem->product) {
            $minimum  = (int) ($cartItem->product->minimum ?? 1);
            $quantity = (int) ($data['quantity'] ?? $cartItem->quantity);
            if ($minimum > 1 && $quantity < $minimum) {
                throw new Exception(trans('front/common.minimum_required', ['min' => $minimum]));
            }
        }
    ...
    {
        // 先删除已有的选项值
        \NiceShoply\Common\Models\Cart\OptionValue::where('cart_item_id', $cartItem->id)->delete();
        ...
        foreach ($options as $optionId => $optionValueIds) {
            ...
            foreach ($optionValueIds as $optionValueId) {
                // 获取选项和选项值信息
                $option      = \NiceShoply\Common\Models\Option::find($optionId);
                $optionValue = \NiceShoply\Common\Models\OptionValue::find($optionValueId);
                ...
                // 获取产品选项值配置中的价格调整
                $productOptionValue = \NiceShoply\Common\Models\Product\OptionValue::where([
                ...
                // 创建购物车选项值记录
                \NiceShoply\Common\Models\Cart\OptionValue::create([
