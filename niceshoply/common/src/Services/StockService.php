...
class StockService extends BaseService
{
    ...
    public function checkStock(string $skuCode, int $quantity, bool $forceCheck = false): bool
    {
        ...
        // Warehouse mode: check total across all active warehouses
        if (system_setting('warehouse_enabled', false)) {
            $available = StockRepo::getInstance()->getTotalAvailable($skuCode);
    ...
    public function getAvailableStock(string $skuCode, bool $forceCheck = false): int
    {
        ...
        // Warehouse mode: sum across all active warehouses
        if (system_setting('warehouse_enabled', false)) {
            return max(0, StockRepo::getInstance()->getTotalAvailable($skuCode));
        }
