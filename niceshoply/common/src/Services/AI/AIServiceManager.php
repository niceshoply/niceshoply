...
class AIServiceManager
{
     ...
     *
     * 解析逻辑：
     * 1. 优先使用 options['model'] 指定的模型；
     * 2. 否则使用后台设置的图片模型 ai_image_model；
     * 3. 否则回退到默认 image 用途映射 / 默认模型。
     * 仅支持实现 AIImageServiceInterface 的适配器。
     *
     * @param  string  $prompt  图片描述
     * @param  array  $options  size、quality、model、save_path 等
     * @return array{url: string, path: string, model: string}
     * @throws \RuntimeException 生成失败或模型不支持图片
     */
    public function generateImage(string $prompt, array $options = []): array
    {
        $model = $options['model']
            ?? system_setting('ai_image_model')
            ?: $this->getModelForPurpose('image');

        $config  = $this->getModelConfig($model);
        $service = AIServiceFactory::make($model, $config);

        if (! $service instanceof AIImageServiceInterface) {
            throw new \RuntimeException("AI 模型不支持图片生成：{$model}");
        }

        $prompt = apply_filters('ai.generate_image_prompt', $prompt, $model);
        $result = $service->generateImage($prompt, $options);

        // 将结果持久化到 media 磁盘，返回可访问 URL
        $savePath = trim($options['save_path'] ?? 'ai-images', '/');
        $disk     = \Illuminate\Support\Facades\Storage::disk('media');

        $binary = null;
        if (! empty($result['url'])) {
            $binary = @file_get_contents($result['url']);
        } elseif (! empty($result['b64_json'])) {
            $binary = base64_decode($result['b64_json']);
        }

        if ($binary === false || $binary === null) {
            throw new \RuntimeException('AI 图片下载或解码失败');
        }

        $fileName = $savePath.'/'.date('Ymd').'/'.\Illuminate\Support\Str::random(32).'.png';
        $disk->put($fileName, $binary);

        return apply_filters('ai.generate_image_result', [
    ...
    private function loadConfig(): array
    {
        ...
        $defaultConfig = [
            ...
            'models'        => [
                ...
                // 标准 OpenAI 兼容通道（可对接 OpenAI / Azure / OpenRouter / 本地 Ollama 等）
                'laravel_ai' => [
                    'enabled'     => system_setting('laravel_ai_enabled', false),
                    'api_key'     => system_setting('laravel_ai_api_key', ''),
                    'base_url'    => system_setting('laravel_ai_base_url', 'https://api.openai.com'),
                    'model'       => system_setting('laravel_ai_model', 'gpt-4o-mini'),
                    'max_tokens'  => 1000,
                    'temperature' => 0.7,
                ],
                'deepseek' => [
