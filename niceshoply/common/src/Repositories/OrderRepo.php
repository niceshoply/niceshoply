...
class OrderRepo extends BaseRepo
{
    ...
    public static function getCriteria(): array
    {
        ...
        return [
            ['name' => 'number', 'type' => 'input', 'label' => trans('console/order.number')],
            ['name' => 'customer_name', 'type' => 'input', 'label' => trans('console/order.customer_name')],
            ['name' => 'email', 'type' => 'input', 'label' => trans('console/order.email')],
            ['name' => 'telephone', 'type' => 'input', 'label' => trans('console/order.telephone')],
            [
                'name'    => 'shipping_method_code', 'type' => 'select', 'label' => trans('console/order.shipping_method_name'),
                'options' => $shippingMethods, 'options_key' => 'code', 'options_label' => 'name',
            ],
            [
                'name'    => 'billing_method_code', 'type' => 'select', 'label' => trans('console/order.billing_method_name'),
                'options' => $billingMethods, 'options_key' => 'code', 'options_label' => 'name',
            ],
            [
                'name'        => 'status', 'type' => 'select', 'label' => trans('console/order.status'), 'options' => $statuses,
                'options_key' => 'status', 'options_label' => 'name',
            ],
            ['name' => 'total', 'type' => 'range', 'label' => trans('console/order.total')],
            ['name' => 'created_at', 'type' => 'date_range', 'label' => trans('console/order.created_at')],
        ];
    }
    ...
    private function handleData($requestData): array
    {
        ...
        return [
            ...
            'currency_snapshot_at'   => now(),
            'customer_tax_id'        => $requestData['customer_tax_id'] ?? ($requestData['reference']['customer_tax_id'] ?? null),
            'proforma_number'        => $requestData['proforma_number'] ?? null,
            'destination_tax_amount' => (float) ($requestData['destination_tax_amount'] ?? 0),
