...
class Tax
{
    ...
    {
        $storeCountryId = (int) system_setting('country_id');
        $isCrossBorder  = $countryId > 0 && $storeCountryId > 0 && $countryId !== $storeCountryId;
        $cacheKey       = "{$type}-{$countryId}-{$stateId}-".(int) $isCrossBorder;

        if (isset(self::$taxRules[$cacheKey])) {
            return self::$taxRules[$cacheKey];
        }
        ...
        $builder = TaxRule::query()
            ...
            // 跨境：目的国≠店铺国时仅匹配 cross_border 规则，否则匹配国内规则
            ->where('rule.cross_border', $isCrossBorder)
            ->orderBy('rule.priority');

        $data = $builder->get();

        self::$taxRules[$cacheKey] = $data;
    ...
    /**
     * $tax = NiceShoply\Common\Libraries\Tax::getInstance();
