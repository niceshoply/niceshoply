...
use NiceShoply\Common\Libraries\ApiHook;
use NiceShoply\Common\Libraries\Currency;
use NiceShoply\Common\Libraries\ViewHook;
use NiceShoply\Common\Libraries\Weight;
...
use NiceShoply\Common\Services\ImageService;
use NiceShoply\Common\Services\StorageService;
use NiceShoply\Common\Support\Registry;
...
if (! function_exists('setting')) {
    ...
    {
        return config("nice.{$key}", $default);
    }
}

if (! function_exists('build_asset')) {
    /**
     * 引用 Vite 构建产物，并基于 public/build/manifest.json 的内容哈希做缓存失效。
     *
     * 在保持固定路径 + IIFE 运行时（不改动内联脚本）的前提下，补齐标准 Vite
     * manifest 的「按内容版本化」能力。manifest 缺失时优雅回退为无版本号 URL。
     *
     * @param  string  $path  相对 build 目录的路径，如 'console/js/app.js'
     * @return string
     */
    function build_asset(string $path): string
    {
        ...
        $url  = asset('build/'.$key);
        $hash = $manifest[$key]['hash'] ?? null;

        return $hash ? $url.'?id='.$hash : $url;
    }
}
...
if (! function_exists('is_secure')) {
    ...
    function is_secure(): bool
    {
        ...
        // Laravel 已信任反代时，以 Request 判断为准（比裸 $_SERVER 更可靠）
        if (function_exists('app') && app()->bound('request')) {
            try {
                return request()->isSecure();
            } catch (\Throwable) {
                // 安装/CLI 等场景无 HTTP 请求时忽略
            }
        }
...
if (! function_exists('detect_app_base_url')) {
    /**
     * 安装向导写入 .env 时推断站点根 URL（含协议）。
     * 优先当前 HTTPS 请求；其次沿用部署前 .env 中的 https APP_URL / APP_FORCE_HTTPS。
     */
    function detect_app_base_url(): string
    {
        ...
        if ($envPath && is_file($envPath)) {
            ...
            if (preg_match('/^APP_FORCE_HTTPS\s*=\s*(.+)$/mi', $envContent, $forceMatch)) {
                $forceValue = strtolower(trim($forceMatch[1], " \t\n\r\0\x0B\"'"));
                if (in_array($forceValue, ['true', '1', 'yes', 'on'], true)) {
                    return 'https://'.$host;
                }
            }

            if (preg_match('/^APP_URL\s*=\s*(.+)$/mi', $envContent, $urlMatch)) {
                $existingUrl = rtrim(trim($urlMatch[1], " \t\n\r\0\x0B\"'"), '/');
                if (str_starts_with(strtolower($existingUrl), 'https://')) {
                    return $existingUrl;
                }
            }
        }
...
if (! function_exists('nice_path')) {
     ...
     * @return string
     */
    function nice_path(string $path): string
    {
        return base_path("niceshoply/{$path}");
    }
}
...
if (! function_exists('nice_view')) {
     ...
     * @return mixed
     */
    function nice_view($view = null, array $data = [], array $mergeData = []): mixed
    {
...
if (! function_exists('debug_view')) {
    ...
    {
        // 调试视图会原样 dump 数据，仅在开启调试模式时可用，避免生产环境数据泄露
        if (! config('app.debug')) {
            abort(404);
        }
...
if (! function_exists('create_json_success')) {
    ...
    function create_json_success($data = null): mixed
    {
        ...
        return json_success(console_trans('common.saved_success'), $data);
    }
}
...
if (! function_exists('read_json_success')) {
    ...
    function read_json_success($data = null): mixed
    {
        ...
        return json_success(console_trans('common.read_success'), $data);
    }
}
...
if (! function_exists('update_json_success')) {
    ...
    function update_json_success($data = null): mixed
    {
        ...
        return json_success(console_trans('common.updated_success'), $data);
    }
}
...
if (! function_exists('delete_json_success')) {
    ...
    function delete_json_success($data = null): mixed
    {
        ...
        return json_success(console_trans('common.deleted_success'), $data);
    }
}
...
if (! function_exists('submit_json_success')) {
    ...
    function submit_json_success($data = null): mixed
    {
        ...
        return json_success(console_trans('common.submitted_success'), $data);
    }
}
...
if (! function_exists('json_success')) {
    ...
    function json_success($message, $data = null): mixed
    {
        ...
        $debugBar = config('app.debug') && request()->has('bar');
        if ($debugBar) {
            return view('console::debugbar', ['data' => $json]);
        }
...
if (! function_exists('storage_service')) {
     ...
     */
    function storage_service(): StorageService
    {
...
if (! function_exists('image_resize')) {
    ...
    {
        // Cloud images: skip resize by default (CDN handles it), unless explicitly enabled
        if (Str::startsWith($image, 'http') && ! system_setting('cloud_image_resize', false)) {
            return $image;
        }
...
if (! function_exists('console_logo_url')) {
    /**
     * 后台 Logo 完整 URL（来自设置 - 基本信息 - 后台 Logo）
     */
    function console_logo_url(): ?string
    {
        $logo = trim((string) system_setting('console_logo', ''));
        if ($logo === '') {
            return null;
        }

        try {
            return image_origin($logo);
        } catch (Throwable) {
            return null;
        }
    }
...
if (! function_exists('safe_front_home_url')) {
    /**
     * 安全获取前台首页 URL（安装锁/路由缓存未就绪时回退到站点根路径，避免 RouteNotFoundException）。
     */
    function safe_front_home_url(): string
    {
        try {
            if (hide_url_locale() || locales()->isEmpty()) {
                if (\Illuminate\Support\Facades\Route::has('front.home.index')) {
                    return route('front.home.index');
                }
            } else {
                $localeRoute = front_locale_code().'.front.home.index';
                if (\Illuminate\Support\Facades\Route::has($localeRoute)) {
                    return route($localeRoute);
                }
                if (\Illuminate\Support\Facades\Route::has('front.home.index')) {
                    return route('front.home.index');
                }
            ...
            // 安装阶段或 route:cache 不完整时，front 路由可能尚未注册
        }
...
if (! function_exists('should_copy_static_file')) {
    ...
    function should_copy_static_file(string $sourceFile, string $destFile): bool
    {
        ...
        // 仅复制普通文件；目录或缺失路径时跳过，避免 copy() 对目录报错导致后台 500
        if ($shouldCopy && is_file($sourceFile)) {
            create_directories(dirname($destFile));

            return copy($sourceFile, $destFile);
        }

        if ($shouldCopy && is_dir($sourceFile)) {
            return is_dir($destFile);
        }

        return file_exists($destFile) && (is_file($destFile) || is_dir($destFile));
    }
}
...
if (! function_exists('niceshoply_version')) {
    /**
     * Generate an asset path for the application.
     *
     * @return string
     */
    function niceshoply_version(): string
    {
        $default = ucfirst(config('niceshoply.edition')).' v'.config('niceshoply.version').'('.config('niceshoply.build').')';

        return fire_hook_filter('NiceShoply.version.display', $default);
    }
}

if (! function_exists('niceshoply_brand_link')) {
    /**
     * 输出 NiceShoply 品牌链接 HTML。
     * 默认展示 Powered by；已购商业授权（去版权/白标）时隐藏或替换为自定义品牌。
     *
     * @return string
     */
    function niceshoply_brand_link(): string
    {
        $licenseService = \NiceShoply\Common\Services\License\BrandLicenseService::getInstance();

        if ($licenseService->canHideBrand()) {
            $default = $licenseService->getCustomBrandHtml();

            return fire_hook_filter('NiceShoply.brand.link.display', $default);
        }

        if (is_admin()) {
            $default = __('common/powered_by.admin_link');
        } else {
            $default = __('common/powered_by.front_link');
        }
...
if (! function_exists('seller_enabled')) {
    ...
    {
        return class_exists('\NiceShoply\Seller\SellerServiceProvider') && env('SELLER_ENABLED', true);
    }
}
