...
use NiceShoply\RestAPI\FrontApiControllers;
...
Route::get('/announcements', [FrontApiControllers\SettingController::class, 'announcements'])->name('announcements.index');

// 敏感认证端点：按 IP 限流，防撞库 / 防刷 / 防短信轰炸
Route::middleware('throttle:10,1')->group(function () {
    ...
    Route::post('/forgotten/password', [FrontApiControllers\AuthController::class, 'resetPassword'])->name('auth.reset_password');
});

// 验证码类端点更严格限流（短信成本敏感）
Route::middleware('throttle:5,1')->group(function () {
    Route::post('/sms-code', [FrontApiControllers\AuthController::class, 'sendSmsCode'])->name('sms-code');
    Route::post('/forgotten/verify_code', [FrontApiControllers\AuthController::class, 'sendVerifyCode'])->name('auth.send_verify_code');
});

Route::post('/refresh', [FrontApiControllers\AuthController::class, 'refresh'])->name('auth.refresh');
...
// Newsletter 订阅（限流防刷）
Route::middleware('throttle:10,1')->group(function () {
    Route::post('/newsletter/subscribe', [FrontApiControllers\NewsletterController::class, 'subscribe'])->name('newsletter.subscribe');
    Route::post('/newsletter/unsubscribe', [FrontApiControllers\NewsletterController::class, 'unsubscribe'])->name('newsletter.unsubscribe');
});
...
Route::get('/pages/{page}', [FrontApiControllers\PageController::class, 'show'])->name('pages.show');
Route::get('/page-{slug}', [FrontApiControllers\PageController::class, 'showBySlug'])->name('pages.show_by_slug');

Route::middleware(['jwt.auth'])->group(function () {

    Route::post('/logout', [FrontApiControllers\AuthController::class, 'logout'])->name('auth.logout');
    ...
    Route::get('/account/points', [FrontApiControllers\AccountController::class, 'points'])->name('account.points');
    Route::get('/account/point_logs', [FrontApiControllers\AccountController::class, 'pointLogs'])->name('account.point_logs');
    Route::post('/account/device_token', [FrontApiControllers\AccountController::class, 'registerDeviceToken'])->name('account.device_token');
    Route::post('/account/privacy/export', [FrontApiControllers\AccountController::class, 'requestDataExport'])->name('account.privacy.export');
    Route::post('/account/privacy/delete', [FrontApiControllers\AccountController::class, 'requestAccountDelete'])->name('account.privacy.delete');
    ...
    Route::post('/checkout/coupon/apply', [FrontApiControllers\CheckoutController::class, 'applyCoupon'])->name('checkout.coupon.apply');
    Route::post('/checkout/coupon/remove', [FrontApiControllers\CheckoutController::class, 'removeCoupon'])->name('checkout.coupon.remove');
    Route::post('/checkout/points/apply', [FrontApiControllers\CheckoutController::class, 'applyPoints'])->name('checkout.points.apply');
    Route::post('/checkout/points/remove', [FrontApiControllers\CheckoutController::class, 'removePoints'])->name('checkout.points.remove');
    ...
    Route::get('/wallet/transactions', [FrontApiControllers\WalletController::class, 'transactions'])->name('wallet.transactions');
    Route::post('/wallet/withdraw', [FrontApiControllers\WalletController::class, 'withdraw'])->name('wallet.withdraw');
