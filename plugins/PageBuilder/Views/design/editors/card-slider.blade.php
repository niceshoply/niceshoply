      ...
      <div class="section-content">
        <div class="tab-container">
                  ...
                  <div class="products-list" v-loading="loading">
                    <template v-if="screen.products.length">
                      <draggable 
                        ...
                        :animation="330"
                        :item-key="item => screen.products.indexOf(item)"
          ...
          <div class="screen-actions">
            ...
            >
              <el-icon><Plus /></el-icon> @{{ lang.add_screen }}
            </el-button>
            <el-button 
              type="danger" 
              size="small" 
              ...
              :disabled="form.screens.length <= 1"
            >
              <el-icon><Delete /></el-icon> @{{ lang.delete_current_screen }}
            </el-button>
          </div>
        </div>
      </div>
  ...
  Vue.component('module-editor-card-slider', {
    ...
    methods: {
      ...
      tabsValueProductData(tabIndex) {
        ...
        axios.get('api/v1/console/products/names?ids=' + this.form.screens[tabIndex].products.map(e => e.id).join(
          ','), {
          hload: true
        }).then((res) => {
          this.loading = false;
      ...
      handleTabChange(name) {
        this.activeTab = name;
        this.form.activeTab = name;
        this.tabsValueProductData(name);
      },
