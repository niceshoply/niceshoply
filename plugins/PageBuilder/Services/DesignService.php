...
use NiceShoply\Common\Libraries\Link;
...
{
    /**
     * 入库时需要按富文本白名单净化的字段（保留安全 HTML）。
     * 这些字段会在前台以 {!! !!} 原样输出，必须做 XSS 过滤。
     */
    private const RICH_TEXT_FIELDS = ['content', 'data'];

    /**
     * 入库时需要剥离全部标签的纯文本字段。
     * 这些字段虽然以多语言数组存储，但语义上是纯文本（标题/副标题/说明等）。
     */
    private const PLAIN_TEXT_FIELDS = ['title', 'subtitle', 'floor', 'description', 'button_text', 'text', 'sub_text', 'name'];
    ...
    public function handleRequestModules(array $modulesData): array
    {
        ...
        foreach ($modulesData as $index => $moduleData) {
            ...
            // 对模块内容做 XSS 净化：富文本字段走白名单过滤，纯文本字段剥离标签，
            // 防止管理员（或被劫持的请求）注入 <script>、onerror 等危险内容到前台。
            if (isset($moduleData['content']) && is_array($moduleData['content'])) {
                $moduleData['content'] = $this->purifyModuleContent($moduleData['content']);
            }
     ...
     * 递归净化模块内容数组。
     *
     * - 富文本字段（content/data）：使用 HtmlPurifyService::clean 做白名单净化，保留安全 HTML；
     * - 纯文本字段（title/subtitle 等）：使用 HtmlPurifyService::strip 剥离全部标签；
     * - 多语言数组（如 ['zh_cn' => '...', 'en' => '...']）：逐语言净化；
     * - 嵌套数组（images / imageTextItems 等）：递归处理。
     *
     * @param  array  $content  模块内容数组
     * @return array
     */
    private function purifyModuleContent(array $content): array
    {
        foreach ($content as $key => $value) {
                ...
                if ($this->isLocaleMap($value)) {
                    ...
                    if (in_array($key, self::RICH_TEXT_FIELDS, true)) {
                        $content[$key] = array_map(
                            fn ($item) => is_string($item) ? HtmlPurifyService::clean($item) : $item,
                            $value
                        );
                    } elseif (in_array($key, self::PLAIN_TEXT_FIELDS, true)) {
                        $content[$key] = array_map(
                            fn ($item) => is_string($item) ? HtmlPurifyService::strip($item) : $item,
                            $value
                        );
                    }
                } else {
                    // 普通嵌套数组（含数字索引数组）：递归净化
                    $content[$key] = $this->purifyModuleContent($value);
                }
            ...
            if (is_string($value)) {
                if (in_array($key, self::RICH_TEXT_FIELDS, true)) {
                    $content[$key] = HtmlPurifyService::clean($value);
                } elseif (in_array($key, self::PLAIN_TEXT_FIELDS, true)) {
                    $content[$key] = HtmlPurifyService::strip($value);
                }
            }
        }
    ...
    /**
     * 判断数组是否为「多语言字符串映射」（键为语言代码、值为字符串）。
     *
     * 例如 ['zh_cn' => '...', 'en' => '...']。用于区分多语言文本字段与
     * 普通的配置数组 / 数字索引数组（如 images 列表）。
     *
     * @param  array  $array
     * @return bool
     */
    private function isLocaleMap(array $array): bool
    {
        if (empty($array) || $this->isNumericArray($array)) {
            return false;
        }

        foreach ($array as $value) {
            // 只要存在非字符串值，即视为普通配置数组而非语言映射
            if (! is_string($value)) {
                return false;
            }
        }
