...
===========================================================================
【文件说明】
  商品自定义选项选择器组件，支持三种交互类型：
    select（下拉框）、radio（单选按钮卡片）、checkbox（多选复选框卡片）。
  每个选项值可配置：价格调整（price_adjustment）、库存数量（quantity）、图片。
  选项选择后实时更新页面价格显示，并对必选项进行验证（加购时阻止未选择的提交）。
  由 products/show.blade.php 通过 @include('products.components._options') 引入。

【来源视图】
  products/show.blade.php

【可用变量】（继承自父视图 + 控制器注入）
  $productOptions        — Collection|null   当前商品的选项集合（ProductOption 模型）
                           关联: $productOption->option（Option 模型）
  $product               — Product 模型     商品对象
    $product->productOptionValues : Collection  商品选项值集合，用于筛选当前 option_id 的值
  $sku                   — array             当前激活 SKU 快照，用于获取基础价格 $sku['price']

【Option 模型属性】
  $option->id            — int    选项 ID
  $option->type          — string 选项类型：'select' / 'radio' / 'checkbox'
  $option->required      — bool   是否必选（必选时加购会触发前端验证）
  $option->currentName   — string 当前语言下的选项名称
  $option->description   — array  选项说明（多语言 JSON 数组，如 ['zh'=>'...','en'=>'...']）

【ProductOptionValue 模型属性】
  $productOptionValue->optionValue        — OptionValue 模型（含 id, currentName, image）
  $productOptionValue->price_adjustment   — float  价格调整量（正为加价，负为减价）
  $productOptionValue->quantity           — int    该选项值的库存

【Sections / Blocks】
  footer — 追加（@push）选项交互 JS + 移动端卡片样式 CSS

【辅助函数】
  currency_format($price)        — 按当前货币格式化价格（带符号）
  image_resize($path)            — 生成选项图片 URL（使用默认尺寸）
  app()->getLocale()             — 获取当前应用语言 code（用于多语言 description）

【JS 全局函数（@push footer）】
  window.updateBasePrice(price)   — 供规格切换组件（_variants）调用，更新基础价格后重算总价
  window.validateRequiredOptions() — 供 show.blade.php 加购按钮事件调用，校验必选项
  updateProductPrice()            — 内部函数，遍历所有已选项计算价格调整并更新价格显示
  updateCurrentSelectionDisplay() — 更新"当前选择摘要"卡片（.current-selection-summary）
  validateRequiredOptions()       — 内部函数，检查必选项并为未选组添加 .has-error 样式

【插件钩子】
  无（此组件无 @hookinsert 钩子点，如需扩展请在父视图钩子中操作）

【自定义建议】
  - 新增选项类型（如颜色色块）：在 @elseif($optionType === 'xxx') 块中扩展新类型逻辑。
  - 修改卡片宽度或样式：调整 .mobile-option-item 及媒体查询断点。
  - 价格显示逻辑：修改 updateProductPrice() 函数，window.inno.formatCurrency() 为全局货币格式化方法。
  - 如需服务端验证价格：在加购接口中同样验证 options 中每项的 price_adjustment。
===========================================================================
--}}
